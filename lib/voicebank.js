...
// MeloTTS ("HD") voices are a directory of a few files, not a single .onnx.
const MELO_FILE_PATTERN = /^[A-Za-z0-9._-]+\.(onnx|txt)$/;
const MODEL_DIR_PATTERN = /^[a-z0-9_]{1,64}$/;
// The files a MeloTTS voice pack always needs: the VITS model and its BERT.
const MELO_REQUIRED_FILES = ['model.onnx', 'bert.onnx'];
...
// MeloTTS voices install into their own directory under voices/.
...
// Download a single engine binary (sha256 + size pinned) to its bin dir.
