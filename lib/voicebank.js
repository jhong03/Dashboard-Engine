...
function engineOf(voice) {
  if (voice && voice.engine === 'melotts') return 'melotts';
  if (voice && voice.engine === 'kokoro') return 'kokoro';
  return 'piper';
}
...
function validateVoice(raw, warnings) {
  ...
  if (raw.engine === 'kokoro') return validateKokoroVoice(raw, where, warnings);
  const checks = [
...
// A Kokoro voice is lightweight: every Kokoro voice shares ONE downloaded model
// (the engine + kokoro-v1.0.onnx + voices-v1.0.bin), so a voice entry only names
// a voice within that model and its language. No per-voice download/sha256 — the
// pinned files belong to the shared engine, installed once.
const KOKORO_VOICE_PATTERN = /^[a-z]{2}_[a-z]+$/;   // e.g. bm_george, em_alex
const KOKORO_LANG_PATTERN = /^[a-z]{2}(-[a-z]{2})?$/; // e.g. en-gb, es, ja

function validateKokoroVoice(raw, where, warnings) {
  const checks = [
    [typeof raw.id === 'string' && ID_PATTERN.test(raw.id), 'invalid id'],
    [typeof raw.displayName === 'string' && raw.displayName.length <= 80, 'invalid displayName'],
    [typeof raw.voiceName === 'string' && KOKORO_VOICE_PATTERN.test(raw.voiceName), 'invalid kokoro voiceName'],
    [typeof raw.langCode === 'string' && KOKORO_LANG_PATTERN.test(raw.langCode), 'invalid kokoro langCode'],
  ];
  for (const [ok, reason] of checks) {
    if (!ok) { warnings.push(`Dropped ${where}: ${reason}.`); return null; }
  }
...
// The Piper counterpart of resolveModelDir: where a Piper voice's .onnx actually
// is FOR READING/synthesis — prefer the downloaded copy in user-data, else a
// copy BUNDLED with the app (a Steam-depot English male at appRoot/voices/<model>).
// The downloader still writes only to user-data (modelPathFor); this is the
// read-side counterpart. Falls back to the user-data path when neither exists so
// callers correctly report "not installed".
function resolvePiperModelPath(appRoot, voice) {
  const userPath = modelPathFor(appRoot, voice); // writable user-data location
  if (fs.existsSync(userPath) && fs.existsSync(`${userPath}.json`)) return userPath;
  const appPath = appRoot ? path.join(appRoot, 'voices', voice.model) : null;
  if (appPath && fs.existsSync(appPath) && fs.existsSync(`${appPath}.json`)) return appPath;
  return userPath;
}

function isInstalled(appRoot, voice) {
  if (engineOf(voice) === 'kokoro') {
    // Every Kokoro voice is present once the shared engine + model are.
    try { return require('./kokoro').isReady(appRoot); } catch { return false; }
  }
  ...
  const p = resolvePiperModelPath(appRoot, voice); // downloaded OR depot-bundled
...
// Download one file, RESUMABLE. A partial from an interrupted attempt is kept as
// `<dest>.part`; the next call re-hashes it and asks the server (HTTP Range) for
// only the remaining bytes — so a dropped connection on a big file doesn't restart
// from zero. Verified by size + sha256 before it's renamed into place. On a network
// cut the `.part` is KEPT (so it resumes); only a checksum mismatch deletes it.
async function downloadStream(url, destPath, expected, onProgress) {
  const partPath = `${destPath}.part`;
  let hash = crypto.createHash('sha256');
  let received = 0;

  // Resume: re-hash any existing partial and continue from its end.
  let resumeFrom = 0;
  try {
    const st = fs.statSync(partPath);
    if (st.size > 0 && (!expected.sizeBytes || st.size < expected.sizeBytes)) {
      await new Promise((resolve, reject) => {
        fs.createReadStream(partPath)
          .on('data', (c) => { hash.update(c); received += c.length; })
          .on('end', resolve)
          .on('error', reject);
      });
      resumeFrom = received;
    }
  } catch { /* no partial to resume from */ }
  ...
  // If we asked to resume but the server sent the whole file (200, not 206), start
  // over: forget the partial's bytes and truncate it.
  let append = resumeFrom > 0 && res.status === 206;
  if (resumeFrom > 0 && !append) { hash = crypto.createHash('sha256'); received = 0; }
  ...
  try {
    ...
    if (expected.sizeBytes && received !== expected.sizeBytes) {
      // Short read = connection cut. KEEP the .part so the next attempt resumes.
      throw new Error(`Download of ${path.basename(destPath)} stopped at ${received}/${expected.sizeBytes} bytes — the connection dropped. It will resume on retry.`);
    }
    if (expected.sha256) {
      const digest = hash.digest('hex');
      if (digest !== expected.sha256) {
        fs.rmSync(partPath, { force: true }); // corrupt — don't resume garbage
    ...
    out.destroy();
    throw err; // NOTE: the .part is intentionally left in place for resume
  }
}

// Only files larger than this are split into parallel range requests — small
// files (configs, tokenizers) aren't worth the extra connections. Overridable so
// tests can force chunking on a tiny file.
const CHUNK_MIN_BYTES = (() => {
  const n = parseInt(process.env.DE_DOWNLOAD_CHUNK_MIN || '', 10);
  return Number.isInteger(n) && n > 0 ? n : 24 * 1024 * 1024;
})();
// How many parallel connections a large file is split across. A single CDN
// connection is throttled well below a typical link's real bandwidth, so N range
// requests roughly multiply throughput — the main HD-download speed win, and the
// fix for "downloads are slow" that doesn't depend on the host. Tunable via
// DE_DOWNLOAD_CONCURRENCY (1–8); default 6.
const CHUNK_CONNECTIONS = (() => {
  const n = parseInt(process.env.DE_DOWNLOAD_CONCURRENCY || process.env.AEGIS_DOWNLOAD_CONCURRENCY || '', 10);
  return Number.isInteger(n) && n >= 1 && n <= 8 ? n : 6;
})();
// Running several connections to a distant CDN means an occasional one drops or
// stalls. Rather than fail the whole install, each range auto-retries a few times,
// RESUMING from the bytes already on disk (so a drop costs a reconnect, not the
// file). An idle connection that sends nothing for IDLE_TIMEOUT_MS is aborted and
// retried too, so a stalled socket can't hang the download. All tunable for tests.
const MAX_RANGE_ATTEMPTS = (() => {
  const n = parseInt(process.env.DE_DOWNLOAD_MAX_ATTEMPTS || '', 10);
  return Number.isInteger(n) && n >= 1 && n <= 20 ? n : 6;
})();
const RETRY_BASE_MS = (() => {
  const n = parseInt(process.env.DE_DOWNLOAD_RETRY_BASE_MS || '', 10);
  return Number.isInteger(n) && n >= 0 ? n : 500;
})();
const IDLE_TIMEOUT_MS = (() => {
  const n = parseInt(process.env.DE_DOWNLOAD_IDLE_MS || '', 10);
  return Number.isInteger(n) && n >= 1000 ? n : 45000;
})();
const sleep = (ms) => new Promise((resolve) => setTimeout(resolve, ms));
const retryBackoffMs = (attempt) => Math.min(10000, RETRY_BASE_MS * Math.pow(2, attempt));

// Download one file. A large, size-known file streams over several parallel
// connections (downloadChunked); everything else uses the single resumable
// stream (downloadStream). If the host answers a Range request with the whole
// file (200 not 206) we clean up and fall back to one stream. Same integrity
// guarantee either way: verified by size + sha256 before the atomic rename.
async function downloadToFile(url, destPath, expected, onProgress) {
  if (expected.sizeBytes && expected.sizeBytes >= CHUNK_MIN_BYTES && CHUNK_CONNECTIONS > 1) {
    try {
      await downloadChunked(url, destPath, expected, onProgress);
      return;
    } catch (err) {
      if (!err || err.code !== 'NO_RANGE') throw err;
      cleanupChunkParts(destPath, CHUNK_CONNECTIONS); // host won't range → one stream
    }
  }
  await downloadStream(url, destPath, expected, onProgress);
}

function cleanupChunkParts(destPath, n) {
  for (let i = 0; i < n; i++) { try { fs.rmSync(`${destPath}.c${i}`, { force: true }); } catch { /* ignore */ } }
}

// Download the byte range [start, end) of `url` into partFile, RESUMABLE: an
// existing partFile continues from its length (Range asks only for the rest).
// onProgress(bytesOfThisRange) fires as it grows. Throws {code:'NO_RANGE'} if the
// server answers a Range request with the whole file (200) instead of 206 — so
// the caller can fall back to a single stream.
async function downloadRangeToFile(url, partFile, start, end, onProgress) {
  const rangeLen = end - start;
  let have = 0;
  try { have = Math.min(fs.statSync(partFile).size, rangeLen); } catch { /* fresh */ }
  if (have >= rangeLen) { if (onProgress) onProgress(rangeLen); return; }
  // Abort the connection if it goes idle (no bytes) — a stalled socket must not
  // hang the whole download; the caller retries it (resuming from what's on disk).
  const ac = new AbortController();
  let idle = setTimeout(() => ac.abort(), IDLE_TIMEOUT_MS);
  let res;
  try {
    res = await fetch(url, { headers: { Range: `bytes=${start + have}-${end - 1}` }, signal: ac.signal });
  } catch (err) { clearTimeout(idle); throw err; }
  if (res.status !== 206 || !res.body) {
    clearTimeout(idle);
    throw Object.assign(new Error('server ignored Range'), { code: 'NO_RANGE' });
  }
  const out = fs.createWriteStream(partFile, { flags: have > 0 ? 'a' : 'w' });
  try {
    for await (const chunk of res.body) {
      clearTimeout(idle); idle = setTimeout(() => ac.abort(), IDLE_TIMEOUT_MS);
      let buf = chunk;
      if (have + buf.length > rangeLen) buf = buf.subarray(0, rangeLen - have); // guard a server overshoot
      have += buf.length;
      if (onProgress) onProgress(have);
      if (!out.write(buf)) await new Promise((resolve) => out.once('drain', resolve));
      if (have >= rangeLen) break;
    }
    clearTimeout(idle);
    await new Promise((resolve, reject) => out.end((err) => (err ? reject(err) : resolve())));
  } catch (err) {
    clearTimeout(idle);
    out.destroy();
    throw err; // partFile KEPT for resume
  }
}

// Download one range with automatic resume-retry. A dropped/reset/idle connection
// (common with many parallel streams to a far CDN) is retried up to
// MAX_RANGE_ATTEMPTS times with exponential backoff, each attempt resuming from
// the bytes already on disk — so a flaky connection self-heals instead of failing
// the install. NO_RANGE is NOT retried; it bubbles up for the single-stream
// fallback. On exhaustion it throws, keeping the part for a later manual retry.
async function fetchRangeResilient(url, partFile, start, end, onProgress) {
  const rangeLen = end - start;
  let lastErr = null;
  for (let attempt = 0; attempt < MAX_RANGE_ATTEMPTS; attempt++) {
    try {
      await downloadRangeToFile(url, partFile, start, end, onProgress);
    } catch (err) {
      if (err && err.code === 'NO_RANGE') throw err;
      lastErr = err;
    }
    let done = 0;
    try { done = fs.statSync(partFile).size; } catch { /* nothing yet */ }
    if (done >= rangeLen) return; // complete
    lastErr = lastErr || new Error('connection closed early');
    if (attempt < MAX_RANGE_ATTEMPTS - 1) await sleep(retryBackoffMs(attempt));
  }
  throw lastErr;
}

// Multi-connection download of a large file: split [0,size) into N contiguous
// ranges fetched in parallel (each to its own `<dest>.c<i>` part, individually
// resumable), then concatenate → sha256-verify → atomic rename. On the first
// error the in-flight ranges settle and their parts are KEPT so a retry resumes
// each range where it stopped.
async function downloadChunked(url, destPath, expected, onProgress) {
  const size = expected.sizeBytes;
  const n = Math.min(CHUNK_CONNECTIONS, Math.max(1, Math.floor(size / CHUNK_MIN_BYTES)));
  if (n <= 1) throw Object.assign(new Error('too small to chunk'), { code: 'NO_RANGE' }); // let caller stream it
  const base = Math.floor(size / n);
  const parts = [];
  for (let i = 0; i < n; i++) {
    parts.push({ start: i * base, end: i === n - 1 ? size : (i + 1) * base, file: `${destPath}.c${i}` });
  }
  const have = parts.map((p) => { try { return Math.min(fs.statSync(p.file).size, p.end - p.start); } catch { return 0; } });
  const report = () => { if (onProgress) onProgress(have.reduce((a, b) => a + b, 0), size); };
  report();

  let firstErr = null;
  await Promise.all(parts.map((p, i) =>
    fetchRangeResilient(url, p.file, p.start, p.end, (h) => { have[i] = h; report(); })
      .catch((err) => { if (!firstErr) firstErr = err; })));
  if (firstErr) throw firstErr; // NO_RANGE, or a range exhausted its retries — parts kept for resume

  // A range that came up short = a dropped connection. KEEP the parts (so a retry
  // resumes each from where it stopped) and ask for a retry — do NOT fall through
  // to the checksum step, which would delete the parts as "corrupt".
  if (parts.some((p, i) => have[i] < p.end - p.start)) {
    const got = have.reduce((a, b) => a + b, 0);
    throw new Error(`Download of ${path.basename(destPath)} stopped at ${got}/${size} bytes — the connection dropped. It will resume on retry.`);
  }

  // Every range is complete — assemble in order, then verify before placing.
  const partPath = `${destPath}.part`;
  ...
  if (expected.sha256) {
    ...
    if (digest !== expected.sha256) {
      ...
      cleanupChunkParts(destPath, n); // corrupt — don't resume garbage
      throw new Error(`Checksum mismatch for ${path.basename(destPath)} — the file is corrupt or was tampered with. It was NOT installed.`);
    }
  }
  fs.renameSync(partPath, destPath);
  cleanupChunkParts(destPath, n);
}
...
async function downloadMeloVoice(appRoot, voice, onProgress) {
  ...
  // Files download one at a time; the throughput comes from each LARGE file being
  // split across several parallel connections inside downloadToFile (a pack is
  // dominated by two big files, so per-file parallelism bought little — the whole
  // connection budget on the current file is faster). Already-installed files
  // (dest exists = verified) are skipped; a partial one resumes. The summed
  // progress bar covers the whole pack.
  let doneBytes = 0;
  for (const f of files) {
    const dest = path.join(dir, f.name);
    if (fs.existsSync(dest)) { doneBytes += f.sizeBytes; if (onProgress) onProgress(doneBytes, total); continue; }
    fs.mkdirSync(path.dirname(dest), { recursive: true }); // e.g. the bert/ subdir
    const startBytes = doneBytes;
    await downloadToFile(f.url, dest, { sha256: f.sha256, sizeBytes: f.sizeBytes },
      (recv) => { if (onProgress) onProgress(startBytes + recv, total); });
    doneBytes += f.sizeBytes;
    if (onProgress) onProgress(doneBytes, total);
  }
}
...
module.exports = {
  ...
  // Exposed for tests (resumable single-file + parallel pack download).
  downloadToFile,
  downloadChunked,
  downloadMeloVoice,
};
