...
async function downloadRangeToFile(url, partFile, start, end, onProgress) {
  ...
  const res = await fetch(url, { headers: { Range: `bytes=${start + have}-${end - 1}` } });
...
async function downloadChunked(url, destPath, expected, onProgress) {
  ...
  await Promise.all(parts.map((p, i) =>
    downloadRangeToFile(url, p.file, p.start, p.end, (h) => { have[i] = h; report(); })
      .catch((err) => { if (!firstErr) firstErr = err; })));
  if (firstErr) throw firstErr; // NO_RANGE (or a network cut) — caller handles; parts kept for resume
