...
// How many of a pack's files to fetch at once. Hugging Face throttles PER
// connection, so several parallel streams roughly multiply throughput — the main
// download-speed win. Tunable via DE_DOWNLOAD_CONCURRENCY.
...
async function downloadMeloVoice(appRoot, voice, onProgress) {
  ...
  // Per-file received bytes; the summed progress bar covers the whole pack.
  const received = new Array(files.length).fill(0);
  const report = () => { if (onProgress) onProgress(received.reduce((a, b) => a + b, 0), total); };

  // A concurrency pool: N workers pull the next file until the list is drained.
  // Each file is still sha256-verified individually (downloadToFile), an already-
  // completed file (dest exists = it was verified) is skipped, and a partial one
  // resumes. On the FIRST error, workers stop taking NEW files and we await the
  // in-flight ones before throwing — so a retry never races a background download.
  let cursor = 0;
  let firstErr = null;
  const worker = async () => {
    for (;;) {
      ...
      if (fs.existsSync(dest)) { received[i] = f.sizeBytes; report(); continue; } // already installed + verified
      try {
        ...
        await downloadToFile(f.url, dest, { sha256: f.sha256, sizeBytes: f.sizeBytes },
          (recv) => { received[i] = recv; report(); });
        received[i] = f.sizeBytes; report();
      } catch (err) {
