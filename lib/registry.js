...
function validateIndexEntry(raw, warnings) {
  ...
  return {
    ...
    // Optional gallery thumbnail; fetched lazily through the main process.
    preview: isAllowedUrl(raw.preview) ? raw.preview : null,
  };
}

// Preview images are decoration, not content — small cap, delivered as a
// data URI so the renderer's CSP never loads remote resources itself.
const MAX_PREVIEW_BYTES = 512 * 1024;
const PREVIEW_MIME = { '.png': 'image/png', '.jpg': 'image/jpeg', '.jpeg': 'image/jpeg', '.webp': 'image/webp' };

async function fetchPreview(url) {
  if (!isAllowedUrl(url)) return { ok: false, error: 'Not an allowed preview URL.' };
  const ext = (new URL(url).pathname.match(/\.(png|jpe?g|webp)$/i) || [])[0];
  if (!ext) return { ok: false, error: 'Previews must be png/jpg/webp.' };
  try {
    const body = await fetchCapped(url, MAX_PREVIEW_BYTES);
    return { ok: true, uri: `data:${PREVIEW_MIME[ext.toLowerCase()]};base64,${body.toString('base64')}` };
  } catch (err) {
    return { ok: false, error: `Could not fetch the preview (${err.message}).` };
  }
}
