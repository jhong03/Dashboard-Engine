...
function pathForId(userDir, id) {
  ...
  // Defence in depth: only ever serve a recognised audio file that still
  // exists — even if music.json were hand-edited to point elsewhere.
  if (!track || !AUDIO_EXT.has(path.extname(track.path).toLowerCase())) return null;
  return fs.existsSync(track.path) ? track.path : null;
}
