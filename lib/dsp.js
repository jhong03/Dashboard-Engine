...
// ffmpeg discovery mirrors piper's: env override, then a writable user-data copy,
// then the BUNDLED binary in the app depot, then bare 'ffmpeg' via PATH. Bundling
// it app-local is what lets the DSP chain (the voice's timbre/character stages)
// run on a clean machine that has no system ffmpeg — otherwise the voice falls
// back to the raw, untuned neural output. appRoot is optional so legacy callers
// (and non-Windows dev) still resolve via PATH.
function findFfmpeg(appRoot) {
  const exe = process.platform === 'win32' ? 'ffmpeg.exe' : 'ffmpeg';
  let userBin = null;
  try { userBin = path.join(require('./paths').userDataDir(), 'bin', 'ffmpeg', exe); } catch { userBin = null; }
  const candidates = [
    process.env.DE_FFMPEG_PATH || process.env.AEGIS_FFMPEG_PATH,
    userBin, // writable user-data location (survives app updates)
    appRoot ? path.join(appRoot, 'bin', 'ffmpeg', exe) : null,
  ].filter(Boolean);
  for (const candidate of candidates) {
    if (fs.existsSync(candidate)) return candidate;
  }
  return exe; // resolved via PATH by spawn; applyDsp reports a clean error if absent
}
