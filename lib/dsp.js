...
const RESONANCE_BELL_HZ = 300;   // low-mid body / "magnetic" chest resonance
const RESONANCE_BELL_Q = 1;
...
function timbreStages(timbre) {
  ...
  if (isActive(timbre.resonance)) {
    stages.push(`equalizer=f=${RESONANCE_BELL_HZ}:t=q:w=${RESONANCE_BELL_Q}:g=${timbre.resonance.toFixed(2)}`);
  }
  ...
  // timbre.breath is a PARALLEL noise branch (split + mix), not a serial stage —
  // it's handled in buildBreathComplex / applyDsp, not here.
  return stages;
}
 ...
 */
// The serial voice stages (everything except the final loudnorm). Shared by the
// plain -af graph and the breath filter_complex so both process the voice identically.
function serialStages(profile, sampleRate) {
  return [
...
function buildFilterGraph(profile, sampleRate) {
  return [...serialStages(profile, sampleRate), LOUDNORM].join(',');
}

// ── Breath (parallel noise branch) ──────────────────────────────────────────
// A soft bed of band-limited noise GATED by the voice, so aspirated "air" only
// sounds while speaking and rises/falls with it. It needs split + mix, so it can't
// be a plain -af stage — buildBreathComplex assembles a filter_complex used ONLY
// when breath > 0. At 0, applyDsp keeps the unchanged -af chain (zero regression).
const BREATH_HP_HZ = 1800;          // the airy breath band
const BREATH_LP_HZ = 6500;
const BREATH_GATE_THRESHOLD = 0.03; // voice level (linear) that opens the breath gate

function breathAmount(profile) {
  return (profile && profile.timbre && typeof profile.timbre.breath === 'number') ? profile.timbre.breath : 0;
}

// filter_complex ending in [out]. durationSec = the input voice length; the noise is
// generated a touch longer and amix trims to the shorter, so reverb tails aren't cut.
function buildBreathComplex(profile, sampleRate, durationSec) {
  const pre = serialStages(profile, sampleRate);
  const chain = pre.length ? `${pre.join(',')},` : '';
  const breath = profile.timbre.breath;                  // 0..1
  const airVol = (0.02 + 0.11 * breath).toFixed(4);       // noise-bed level (start point; tune by ear)
  const dur = (Math.max(0.05, durationSec) + 1).toFixed(3);
  return (
    `[0:a]${chain}asplit=2[dry][key];` +
    `anoisesrc=r=${sampleRate}:d=${dur}:c=pink:a=1,` +
      `highpass=f=${BREATH_HP_HZ},lowpass=f=${BREATH_LP_HZ},volume=${airVol}[nb];` +
    `[nb][key]sidechaingate=threshold=${BREATH_GATE_THRESHOLD}:ratio=2:range=0.06:attack=8:release=140[air];` +
    `[dry][air]amix=inputs=2:normalize=0:duration=shortest[mix];` +
    `[mix]${LOUDNORM}[out]`
  );
}
...
function applyDsp(pcm, sampleRate, profile, ffmpegPath) {
  return new Promise((resolve, reject) => {
    // Breath needs a parallel branch (filter_complex); everything else is a simple
    // -af chain. Only pay for the complex path when breath is actually dialed in.
    const io = [
    ...
    let filterArgs;
    if (isActive(breathAmount(profile))) {
      const durationSec = pcm.length / 2 / sampleRate; // s16le mono: 2 bytes/sample
      filterArgs = ['-filter_complex', buildBreathComplex(profile, sampleRate, durationSec), '-map', '[out]'];
    } else {
      filterArgs = ['-af', buildFilterGraph(profile, sampleRate)];
    }
    const args = ['-hide_banner', '-loglevel', 'error', ...io, ...filterArgs, ...out];
...
function findFfmpeg() {
  return process.env.DE_FFMPEG_PATH || process.env.AEGIS_FFMPEG_PATH || 'ffmpeg';
}
