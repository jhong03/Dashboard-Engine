...
// The monitor the wallpaper renders on, stored as a geometry SIGNATURE, not just
// an Electron display id. Electron's `display.id` is a synthetic integer that is
// NOT stable across a reboot / GPU reinit on Windows — the same physical monitor
// gets a different id, so a saved id matches nothing next boot and the pin is
// lost (the "reverts to primary on restart" bug). We keep the id as a fast path
// but also record position + size (+ scaleFactor/label) so main can re-identify
// the monitor by where it is even when the id shuffles. null = follow primary.
function readDisplayPin(raw) {
  const p = raw && raw.displayPin;
  if (p && typeof p === 'object' && Number.isInteger(p.id)) {
    const num = (v) => (Number.isFinite(Number(v)) ? Math.round(Number(v)) : null);
    return {
      id: p.id,
      x: num(p.x), y: num(p.y),
      width: num(p.width), height: num(p.height),
      scaleFactor: Number.isFinite(Number(p.scaleFactor)) ? Number(p.scaleFactor) : null,
      label: typeof p.label === 'string' ? p.label : '',
    };
  }
  // Back-compat: an older build stored a bare integer `displayId`. Carry it over
  // as an id-only pin; main upgrades it to a full signature the first time that
  // monitor is present in a session (so it survives the next reboot).
  if (raw && Number.isInteger(raw.displayId)) {
    return { id: raw.displayId, x: null, y: null, width: null, height: null, scaleFactor: null, label: '' };
  }
  return null;
}
...
// Editor sidebar widths (palette + inspector), in px. Each null → the CSS default.
// Clamped to sane absolute ranges; the editor also caps them at runtime.
function readEditorLayout(raw) {
  const src = raw && raw.editorLayout;
  const pick = (v, min, max) => {
    const n = Number(v);
    return Number.isFinite(n) && n >= min && n <= max ? Math.round(n) : null;
  };
  return {
    pal: src ? pick(src.pal, 130, 420) : null,
    insp: src ? pick(src.insp, 220, 640) : null,
  };
}
...
function load(userDir) {
  try {
    ...
    return {
      ...
      displayPin: readDisplayPin(raw),
      onboarded: raw.onboarded === true,
      weatherLocation: readWeatherLocation(raw),
      healthVoiceAlerts: raw.healthVoiceAlerts === true,
      assistantSpeed: normalizeAssistantSpeed(raw.assistantSpeed),
      locale: typeof raw.locale === 'string' && LOCALE_PATTERN.test(raw.locale) ? raw.locale : null,
      detailWidth: readDetailWidth(raw),
      editorLayout: readEditorLayout(raw),
    };
  } catch {
    return { activePack: null, performance: { ...DEFAULT_PERFORMANCE }, backgroundMotion: { ...DEFAULT_BACKGROUND_MOTION }, displayPin: null, onboarded: false, weatherLocation: null, healthVoiceAlerts: false, assistantSpeed: 'medium', locale: null, detailWidth: null, editorLayout: { pal: null, insp: null } };
  }
}
...
function getEditorLayout(userDir) {
  return load(userDir).editorLayout;
}

// Merge a partial { pal?, insp? }: apply only the fields that arrive VALID, keeping
// the current value for anything absent or out of range. So setting one width never
// wipes the other, and a bad write can't clear a good stored width.
function setEditorLayout(userDir, layout) {
  const settings = load(userDir);
  const cur = settings.editorLayout || { pal: null, insp: null };
  const incoming = readEditorLayout({ editorLayout: layout || {} });
  settings.editorLayout = {
    pal: incoming.pal != null ? incoming.pal : cur.pal,
    insp: incoming.insp != null ? incoming.insp : cur.insp,
  };
  save(userDir, settings);
  return settings.editorLayout;
}
...
// The pinned monitor's signature ({ id, x, y, width, height, scaleFactor, label })
// or null (follow primary). Main resolves it to a live display (by id, else by
// geometry) — see chosenDisplay() in main.js.
function getDisplayPin(userDir) {
  return load(userDir).displayPin;
}

// null clears the choice (follow primary); otherwise pass the full descriptor of
// the chosen display so it can be re-identified after an id shuffle. Drops the
// legacy bare `displayId` key so there's one source of truth.
function setDisplayPin(userDir, pin) {
  const settings = load(userDir);
  settings.displayPin = (pin && Number.isInteger(pin.id)) ? pin : null;
  save(userDir, settings);
  return settings.displayPin;
}

// Read-only back-compat shim for any caller that still wants just the id.
function getDisplayId(userDir) {
  const pin = load(userDir).displayPin;
...
// Assistant response speed — a Low/Medium/High trade-off between reply speed and
// CPU usage, mapped to the local model's thread count (see lib/ipc.js). Default
// Medium. Any unknown value falls back to Medium.
const ASSISTANT_SPEEDS = ['low', 'medium', 'high'];
function normalizeAssistantSpeed(v) {
  return ASSISTANT_SPEEDS.includes(v) ? v : 'medium';
}
function getAssistantSpeed(userDir) {
  return load(userDir).assistantSpeed;
}
function setAssistantSpeed(userDir, level) {
  const settings = load(userDir);
  settings.assistantSpeed = normalizeAssistantSpeed(level);
  save(userDir, settings);
  return settings.assistantSpeed;
}
...
module.exports = {
  ...
  getEditorLayout, setEditorLayout,
  getDisplayPin, setDisplayPin, getDisplayId,
  getOnboarded, setOnboarded,
  getWeatherLocation, setWeatherLocation,
  getHealthVoiceAlerts, setHealthVoiceAlerts,
  getAssistantSpeed, setAssistantSpeed, ASSISTANT_SPEEDS,
  getLocale, setLocale,
};
