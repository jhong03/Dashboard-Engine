...
/**
 * Register a single MAIN-STAGED video (an editor import that isn't in a pack
 * yet) so the editor stage can play it live before save. The path is validated
 * as a real, existing video; returns its depack:// url or null.
 * @returns {string|null}
 */
function registerStagedVideo(absPath) {
  if (!packs.isVideoAsset(absPath) || !fs.existsSync(absPath)) return null;
...
module.exports = { registerPackVideos, registerStagedVideo, pathForId };
