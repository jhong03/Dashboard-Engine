...
function prewarm(text, modelDir, enginePath, opts = {}) {
  // Be polite: if a real synth (a reply/test) is already running or queued, skip.
  // Warming can wait, and starting one now could switch the engine's language
  // out from under an in-flight request (ensureChild respawns per language),
  // rejecting it — the "text came back but no voice" case.
  if (inflight || waiters.length) return;
  const profile = { prosody: { rate: 165, expressiveness: 1, steadiness: 0.5 } };
  try {
    synthesize(text || 'Hello.', profile, modelDir, enginePath, opts).catch(() => { /* best effort */ });
  } catch { /* best effort */ }
}
