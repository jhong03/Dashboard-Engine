...
// The frozen engine ships as a folder with melotts-engine.exe inside. Look in
// (1) an explicit env override, (2) the writable user-data location the
// downloader uses (survives app updates), (3) the app install dir. Returns null
// if not found — callers fall back to Piper/system voice, they never throw.
function findEngine(appRoot) {
  const exe = process.platform === 'win32' ? 'melotts-engine.exe' : 'melotts-engine';
  let userBin = null;
  try { userBin = path.join(require('./paths').userDataDir(), 'bin', 'melotts', exe); } catch { userBin = null; }
  const candidates = [
    process.env.DE_MELOTTS_ENGINE || process.env.AEGIS_MELOTTS_ENGINE,
    userBin,
    appRoot ? path.join(appRoot, 'bin', 'melotts', exe) : null,
  ].filter(Boolean);
...
function isEngineInstalled(appRoot) {
  return findEngine(appRoot) !== null;
}

// A MeloTTS "model" is a directory holding the VITS model.onnx and its sentence
// BERT (bert.onnx); the text frontend + BERT tokenizer ship inside the engine.
function isModelInstalled(modelDir) {
  return (
    fs.existsSync(path.join(modelDir, 'model.onnx')) &&
    fs.existsSync(path.join(modelDir, 'bert.onnx'))
  );
}
...
function killEngine() {
  ...
  child = null;
  childEnginePath = null;
}

function ensureChild(enginePath) {
  if (child && childEnginePath === enginePath) return child;
  // Engine path changed (e.g. after an install) — restart cleanly.
  killEngine();

  const proc = spawn(enginePath, [], { stdio: ['pipe', 'pipe', 'pipe'], windowsHide: true });
  child = proc;
  childEnginePath = enginePath;
