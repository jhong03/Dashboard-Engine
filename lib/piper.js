...
// words-per-minute onto it we need a baseline: the wpm the model produces at
// length-scale 1.0. That varies per voice, so `npm run calibrate` measures it
// and stores it in voices.json as wpmAtScale1; callers pass it through as
// opts.baselineWpm. This constant is only the fallback for uncalibrated
// voices (Stage 1 measured alan ~30% slower than this guess — calibrate!).
...
function lengthScaleForRate(wpm, baselineWpm = BASELINE_WPM_AT_SCALE_1) {
  return clamp(baselineWpm / wpm, LENGTH_SCALE_MIN, LENGTH_SCALE_MAX);
}
...
function piperArgsForProfile(profile, modelPath, baselineWpm, speaker) {
  const prosody = profile.prosody;
  const args = [
    '--model', modelPath,
    '--length-scale', lengthScaleForRate(prosody.rate, baselineWpm).toFixed(3),
  ...
  // Multi-speaker models (e.g. VCTK, which gives us a shippable British MALE
  // voice) need the speaker index; single-speaker models omit it.
  if (Number.isInteger(speaker)) args.push('--speaker', String(speaker));
  return args;
}
...
function findPiper(appRoot) {
  ...
  let userBin = null;
  try { userBin = path.join(require('./paths').userDataDir(), 'bin', 'piper', exe); } catch { userBin = null; }
  const candidates = [
    process.env.DE_PIPER_PATH || process.env.AEGIS_PIPER_PATH,
    userBin, // writable user-data location (survives app updates)
 ...
 * @param {object} [opts] — opts.baselineWpm: the voice's measured wpm at
 *   length-scale 1.0 (voices.json wpmAtScale1); omit for the fallback guess.
 * @returns Promise<{ pcm: Buffer, sampleRate: number }>
 * Rejects with a human-readable Error — no raw stack traces reach the UI.
 */
function synthesize(text, profile, modelPath, piperPath, opts = {}) {
  return new Promise((resolve, reject) => {
    ...
    const args = piperArgsForProfile(profile, modelPath, opts.baselineWpm, opts.speaker);
