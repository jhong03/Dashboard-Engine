...
// ── upload rate-limit pacing ─────────────────────────────────────────────────
// Steam throttles how OFTEN you can submit a Workshop update, and its
// "limit exceeded" result carries NO retry-after — so we can't read the real
// remaining time. Instead we record when we last hit it and count down a
// conservative LOCAL cooldown: it gives the user a concrete "try again in M:SS"
// and stops rapid re-hammering (the very thing that trips the limit). Cleared on
// the next successful publish.
const RATE_LIMIT_COOLDOWN_MS = 10 * 60 * 1000;
function stateFile(userDir) { return path.join(userDir, 'workshop-state.json'); }
function loadState(userDir) {
  try { const v = JSON.parse(fs.readFileSync(stateFile(userDir), 'utf8')); return (v && typeof v === 'object') ? v : {}; }
  catch (e) { return {}; }
}
function saveState(userDir, s) {
  try { fs.mkdirSync(userDir, { recursive: true }); fs.writeFileSync(stateFile(userDir), JSON.stringify(s, null, 2)); }
  catch (e) { /* best-effort — a lost timer just means no countdown */ }
}
function markRateLimited(userDir) { const s = loadState(userDir); s.rateLimitedAt = Date.now(); saveState(userDir, s); }
function clearRateLimited(userDir) { const s = loadState(userDir); if (s.rateLimitedAt) { delete s.rateLimitedAt; saveState(userDir, s); } }
// Milliseconds left on the local cooldown (0 = clear to publish).
function rateLimitRemainingMs(userDir) {
  const at = loadState(userDir).rateLimitedAt;
  if (!at) return 0;
  const rem = RATE_LIMIT_COOLDOWN_MS - (Date.now() - at);
  return rem > 0 ? rem : 0;
}
...
async function publish(appRoot, userDir, opts) {
  ...
  try {
    ...
    // Records the local cooldown + returns a clean, public-safe message with a
    // retry countdown the dialog can display.
    const rateLimited = () => {
      markRateLimited(userDir);
      return { ok: false, rateLimited: true, retryAfterMs: rateLimitRemainingMs(userDir),
        error: 'Too many publishes in a short time. Steam limits how often you can upload — please wait a little while and try again.' };
    };
    ...
    setPublishedItem(userDir, id, itemId.toString());
    clearRateLimited(userDir); // a successful upload means the throttle cleared
    return {
      ok: true,
...
module.exports = {
  ...
  listSubscribed, importSubscribed, subscribe, listMine, getEditableCopy, getPublishedItem, getItemVisibility, rateLimitRemainingMs,
