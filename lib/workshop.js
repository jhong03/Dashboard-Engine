...
async function publish(appRoot, userDir, opts) {
  ...
  try {
    ...
    // Plain updateItem resolves only after the upload COMPLETES (measured ~3 s
    // for a small pack), so deleting the staged content/preview in the finally
    // blocks below is safe. The mapping is sticky on failure so a retry updates
    // the SAME item; to start fresh, delete workshop-items.json.
    ...
    if (/limit exceeded/i.test(msg)) {
      return { ok: false, error: 'Steam is rate-limiting Workshop uploads (too many recent publishes). Give it a while, then try again — the engine reuses one item per pack, so this should stay rare.' };
    }
