...
// Preview-before-subscribe: this binding can't set queryable Steam metadata, and
// Steam serves NO item content until you subscribe — so a voice's compact tuning
// PARAMETERS (numbers + the base-voice id, NEVER any audio) ride in a strippable
// block at the end of the item description. A browsing app decodes it and
// synthesizes a short sample LOCALLY from the licensed, locally-installed base
// voice. Delimited with `%%…%%` (not BBCode/angle brackets) and base64url'd
// (A-Za-z0-9-_) so nothing in it collides with Steam's description handling.
const VOICE_PARAMS_MARK = 'DEVOICE1';
const VOICE_PARAMS_RE = new RegExp(`%%${VOICE_PARAMS_MARK}%%([A-Za-z0-9_-]+)%%`);
const VOICE_PARAMS_STRIP_RE = new RegExp(`\\s*%%${VOICE_PARAMS_MARK}%%[A-Za-z0-9_-]+%%`, 'g');

function encodeVoiceParams(profile) {
  try {
    const p = profiles.sanitizeProfile(profile);
    // Only what synthesis needs — base voice + tuning. No name/author/provenance.
    const slim = { base: p.base, prosody: p.prosody, timbre: p.timbre, character: p.character };
    const b64 = Buffer.from(JSON.stringify(slim), 'utf8').toString('base64url');
    return `%%${VOICE_PARAMS_MARK}%%${b64}%%`;
  } catch (e) { return ''; }
}

function decodeVoiceParams(description) {
  const m = VOICE_PARAMS_RE.exec(String(description || ''));
  if (!m) return null;
  try { return JSON.parse(Buffer.from(m[1], 'base64url').toString('utf8')); } catch (e) { return null; }
}

function stripVoiceParams(description) {
  return String(description || '').replace(VOICE_PARAMS_STRIP_RE, '').trim();
}
...
// The persistent ENGINE process must NEVER open a Steam connection (Steam would
// then mark IT as "playing" instead of the transient Manager session). So the
// engine installs a forwarder that routes every Steam-client op to the session
// process, which owns the client. When no forwarder is set (the session itself, and
// unpackaged dev), the op runs locally. See lib/session-link.js + main.js runSession.
let forwarder = null;
function setForwarder(fn) { forwarder = fn; }
function fwd(name, impl) {
  return (...args) => (forwarder ? forwarder(name, args) : impl(...args));
}
...
function stageContent(userDir, packId, dir, previewPath) {
  ...
  // Strip personal data (e.g. the author's weather location) before upload — the
  // staged copy is what Steam publishes; the author's local pack is untouched.
  fs.writeFileSync(path.join(stageDir, 'pack.json'), packstore.shareableManifest(path.join(dir, 'pack.json')));
...
async function publish(appRoot, userDir, opts) {
  ...
  try {
    ...
    const rateLimited = () => ({ ok: false, error: 'Too many publishes in a short time — please try again later.' });
    ...
    return {
      ...
      adopted,
      note: [
        adopted ? 'Steam wouldn’t create a new Workshop item just now, so your most recent one was updated instead.' : null,
    ...
    if (isLimitExceeded(err)) {
      return { ok: false, error: 'Too many publishes in a short time — please try again later.' };
    }
    ...
    if (/busy/i.test(msg)) {
      return { ok: false, error: 'Steam is busy right now and couldn’t publish. This is usually temporary — please try again in a little while.' };
    }
    return { ok: false, error: 'Couldn’t publish to the Workshop right now. Please try again in a little while.' };
  }
}
...
// Whether the local pack for a published item is an EDITABLE copy — the master you
// can edit + update, not a plain "Add to library" import (which comes in as
// origin:'imported' and can't be published). Mirrors the strict publish gate
// (packstore.isPublishable) for a user pack, without needing appRoot. This is what
// keeps "Get editable copy" available when only a non-editable copy of your own
// published pack exists locally, so you can always recover the master.
function localEditableCopy(userDir, id) {
  if (!localPackExists(userDir, id)) return false;
  const meta = packstore.readMeta(userDir, id);
  if (!meta) return true; // legacy user pack — benefit of the doubt, as isPublishable does
    ...
    for (let page = 1; page <= 10; page++) {
      ...
      for (const it of items) {
        ...
        out.push({
          ...
          // Only surface a local id whose pack is a real EDITABLE copy here — a plain
          // "Add to library" import (origin:'imported') does NOT count, so "Get
          // editable copy" stays available and you can always restore the master.
          localPackId: mapped && localEditableCopy(userDir, mapped) ? mapped : null,
        });
      }
      seen += items.length;
      if (items.length === 0 || seen >= ((res && res.totalResults) || 0)) break;
...
async function publishVoice(appRoot, userDir, opts) {
  ...
  try {
    ...
    // Carry the tuning params so browsers can preview WITHOUT subscribing (see
    // the VOICE_PARAMS_MARK note). Leave room for it inside Steam's 8000 cap.
    const paramsBlock = encodeVoiceParams(profile);
    const room = 8000 - (paramsBlock ? paramsBlock.length + 2 : 0);
    const baseDesc = String(opts && opts.description || '').slice(0, Math.max(0, room));
    const description = paramsBlock ? `${baseDesc}\n\n${paramsBlock}` : baseDesc;
...
async function browseVoices(opts) {
  ...
  // Fallback to NEWEST (not trend): RankedByTrend omits brand-new items, so a bad
  // or missing sort must never default to a view that hides fresh publishes.
  const queryType = SORT_QUERY[(opts && opts.sort)] != null ? SORT_QUERY[opts.sort] : SORT_QUERY.newest;
  try {
    ...
    const res = await c.workshop.getAllItems(
      ...
      // includeLongDescription: the embedded tuning params ride in the full
      // description so a card can be previewed before subscribing.
      { searchText: search || undefined, includeLongDescription: true, requiredTags: [VOICE_TAG] },
    );
    ...
    const items = ((res && res.items) || []).filter(isVoiceItem).map((it) => {
      ...
      return {
        ...
        description: stripVoiceParams(it.description).replace(/\[[^\]]*\]/g, '').slice(0, 200),
        // Tuning params for a local, pre-subscribe sample preview (null on older
        // items published before this feature — the UI just omits the button).
        previewParams: decodeVoiceParams(it.description),
    ...
    for (let page = 1; page <= 10; page++) {
      ...
      for (const it of items) {
        ...
        out.push({
          ...
          url: it.url || `https://steamcommunity.com/sharedfiles/filedetails/?id=${idStr}`,
          previewUrl: typeof it.previewUrl === 'string' ? it.previewUrl : null,
          // Same as dashboards: only an EDITABLE local profile counts, so a plain
          // imported voice doesn't hide "Get editable copy" for your own published one.
          localProfileFile: mappedFile && localFiles.has(mappedFile) && profiles.isPublishable(appRoot, mappedFile) ? mappedFile : null,
        });
      }
      seen += items.length;
      if (items.length === 0 || seen >= ((res && res.totalResults) || 0)) break;
...
async function importVoice(appRoot, userDir, itemId) {
  // Moderation gate: refuse to install a Workshop voice we've confirmed as violating.
  const blocked = moderation.blockReason(appRoot, itemId);
...
// Workshop preview images live on Steam's own CDNs, and the URL always comes from
// Steam's API — so pinning to those hosts never blocks a real preview, but it stops
// a compromised renderer from using this main-side fetch to probe arbitrary or
// internal hosts (SSRF). A missed host just falls back to the placeholder, never a
// crash.
function isSteamPreviewHost(hostname) {
  const h = String(hostname || '').toLowerCase();
  return h.endsWith('.akamaihd.net')          // steamuserimages-a / steamcdn-a
    || h.endsWith('.steamstatic.com')          // akamai/cloudflare steamstatic
    || h.endsWith('.steamusercontent.com')
    || h.endsWith('.steamcontent.com')
    || h.endsWith('.steampowered.com')
    || h.endsWith('.steamcommunity.com');
}

// Fetch a Workshop preview image and return it as a data: URI so the manager's
// CSP (img-src 'self' data:) can show it. Cached by URL; size/type capped; the
// host is pinned to Steam's CDNs (see isSteamPreviewHost).
const previewCache = new Map();
function previewDataUri(url) {
  return new Promise((resolve) => {
    if (typeof url !== 'string') return resolve({ ok: false });
    let parsed;
    try { parsed = new URL(url); } catch (e) { return resolve({ ok: false }); }
    if (parsed.protocol !== 'https:' || !isSteamPreviewHost(parsed.hostname)) return resolve({ ok: false });
...
function importSubscribed(appRoot, userDir, itemId) {
  // Moderation gate: refuse to install a Workshop item we've confirmed as violating.
  const blocked = moderation.blockReason(appRoot, itemId);
  if (blocked) return { ok: false, error: blocked, blocked: true };
  const c = ensureClient();
  if (!c) return { ok: false, error: lastReason || 'Steam is not available.' };
...
module.exports = {
  STEAM_APP_ID, VOICE_TAG, setForwarder,
  // getClient exposes the single, fail-soft Steam client (ensureClient) so other
  // SESSION-side modules — e.g. lib/achievements — reuse it. The engine forwards
  // achievements instead of calling this (it must not open a Steam connection).
  getClient: ensureClient,
  // Local-only helpers (no Steam client) — always run in-process, incl. the engine:
  previewDataUri, workshopPageUrl, getPublishedItem, getPublishedVoiceItem,
  // Steam-client ops — forwarded to the session when a forwarder is installed.
  status: fwd('status', status),
  publish: fwd('publish', publish),
  browse: fwd('browse', browse),
  listSubscribed: fwd('listSubscribed', listSubscribed),
  importSubscribed: fwd('importSubscribed', importSubscribed),
  subscribe: fwd('subscribe', subscribe),
  listMine: fwd('listMine', listMine),
  getEditableCopy: fwd('getEditableCopy', getEditableCopy),
  getItemVisibility: fwd('getItemVisibility', getItemVisibility),
  // Voice profiles (separate Workshop item type).
  publishVoice: fwd('publishVoice', publishVoice),
  browseVoices: fwd('browseVoices', browseVoices),
  listMineVoices: fwd('listMineVoices', listMineVoices),
  getEditableVoice: fwd('getEditableVoice', getEditableVoice),
  importVoice: fwd('importVoice', importVoice),
};
