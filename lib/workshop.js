...
async function publish(appRoot, userDir, opts) {
  ...
  try {
    ...
    try {
      result = await c.workshop.updateItem(itemId, update, STEAM_APP_ID);
    } catch (err) {
      console.log('[workshop] updateItem FAILED:', String((err && err.message) || err), '| had preview:', !!update.previewPath);
