...
async function publish(appRoot, userDir, opts) {
  ...
  try {
    ...
    // Reuse the item created for this pack before; only create one the first
    // time, and record its id IMMEDIATELY — so if the content upload below
    // fails, the next publish updates that same (empty) item instead of
    // creating yet another. This is what stops the blank-item pile-up.
    ...
    // The mapping is sticky on failure: a retry updates the SAME item rather
    // than creating another (that's the whole point). If the item was deleted
    // on Steam, reset by deleting workshop-items.json in the user-data dir.
