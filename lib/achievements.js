...
// The persistent ENGINE installs a forwarder so unlocks route to the session process
// (which owns the Steam client). The engine must never open its own Steam connection.
let forwarder = null;
...
function unlock(name) {
  ...
  // ENGINE: forward to the session (which owns the Steam client). Its forwarder
  // queues the unlock when no session is open and flushes on the next connect.
  // Fire-and-forget + fail-soft — an achievement can never break a feature.
  if (forwarder) {
    unlockedThisSession.add(name);
    try { Promise.resolve(forwarder('unlock', [name])).catch(() => {}); } catch (err) { /* ignore */ }
    return true;
  }

  // SESSION / unpackaged dev: unlock locally against our own Steam client.
  let client = null;
...
module.exports = { unlock, setForwarder };
