...
function registerIpcHandlers(appRoot, userDir, hooks = {}) {
  ...
  ipcMain.handle('aegis:assistant:warmup', (event, trigger) => {
    // Preloading is tied to the response-speed setting: High preloads when the
    // assistant renders on the desktop (fastest first reply, most idle RAM), Medium
    // warms when the chat opens, Low never preloads (lightest — the model loads only
    // when a message is actually sent). A missing/other trigger (e.g. the Manager)
    // always warms, since it's an explicit about-to-use signal.
    const speed = settings.getAssistantSpeed(userDir);
    const warm = trigger === 'render' ? speed === 'high'
      : trigger === 'open' ? (speed === 'high' || speed === 'medium')
      : true;
    if (warm) warmupAssistant(appRoot, userDir);
    return { ok: true };
  });
