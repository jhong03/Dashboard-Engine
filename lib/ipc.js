...
function registerIpcHandlers(appRoot, userDir, hooks = {}) {
  ...
  const POMODORO_ACTIONS = new Set(['start', 'pause', 'toggle', 'reset', 'skip', 'sync']);
  ...
  ipcMain.handle('aegis:pomodoro:control', (event, payload) => {
    ...
    const cfg = (typeof req.cfg === 'object' && req.cfg !== null) ? { cfg: req.cfg } : undefined;
    const state = pomodoro.control(userDir, action, cfg);
