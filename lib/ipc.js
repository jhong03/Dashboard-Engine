...
function registerIpcHandlers(appRoot, userDir, hooks = {}) {
  ...
  ipcMain.handle('aegis:i18n:set', (event, code) => {
    ...
    // The tray menu is rebuilt fresh on each right-click, so it picks up the new
    // language automatically; renderers reload themselves to apply their dict.
    return { ok: true, locale: saved };
  });
