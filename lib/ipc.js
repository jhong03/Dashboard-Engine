...
async function warmupAssistant(appRoot, userDir) {
  try {
    ...
    prewarmAssistantVoice(appRoot, userDir); // warm the TTS engine in parallel (best-effort)
    try { logger.write(userDir, 'PERF', `warmup: voice-prewarm dispatched +${Date.now() - _t}ms ${_mem()}`); } catch (e) { /* ignore */ }
    ...
    try { logger.write(userDir, 'PERF', `warmup: assistant.warmup done +${Date.now() - _t}ms ${_mem()}`); } catch (e) { /* ignore */ }
  } catch { /* best effort */ }
}
...
function registerIpcHandlers(appRoot, userDir, hooks = {}) {
  ...
  ipcMain.handle('aegis:assistant:warmup', async (event, trigger) => {
    ...
    const _tw = Date.now();
    await Promise.all([
      warmupAssistant(appRoot, userDir),      // language model
      synthSpeech('Hello.').catch(() => {}),  // TTS engine (audio discarded)
    ]);
