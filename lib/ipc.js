...
function registerIpcHandlers(appRoot, hooks = {}) {
  ...
  function resetPackWatcher(sender, packId) {
    ...
    let watcher;
    try {
      watcher = fs.watch(packs.packDir(appRoot, packId), { persistent: false }, () => {
  ...
  ipcMain.handle('aegis:packs:list', () => {
    const listed = packs.listPacks(appRoot);
  ...
  ipcMain.handle('aegis:packs:load', (event, id) => {
    ...
    const { pack, warnings } = packs.loadPack(appRoot, id);
    const wallpaper = packs.wallpaperDataUri(appRoot, pack);
    resetPackWatcher(event.sender, pack.id);
    return { ok: true, pack, wallpaper: wallpaper.uri, warnings: [...warnings, ...wallpaper.warnings] };
  });
