...
function registerIpcHandlers(appRoot, userDir, hooks = {}) {
  ...
  ipcMain.handle('aegis:assistant:speak', async (event, text) => {
    markAssistantActive();
    const n = ++speakSeq;
    const out = await synthSpeech(text);
    try {
      const r = out && out.ok ? `ok ${out.pcm ? out.pcm.length : 0}B @${out.sampleRate}` : `FAIL ${out && out.error}`;
      logger.write(userDir, 'INFO', `[dbg speak #${n}] "${String(text || '').slice(0, 24).replace(/\s+/g, ' ')}" -> ${r}`);
    } catch (e) { /* best effort */ }
    return out;
  });

  // Dev diagnostic: the desktop renderer has no visible console in a packaged build,
  // so it forwards debug lines here to land in engine.log.
  ipcMain.handle('aegis:debug:log', (event, msg) => {
    try { logger.write(userDir, 'INFO', `[dbg] ${String(msg || '').slice(0, 400)}`); } catch (e) { /* best effort */ }
    return { ok: true };
  });
