...
function voiceEngineReady(appRoot, voice, env) {
  if (bank.engineOf(voice) === 'melotts') {
    return melotts.isEngineInstalled(appRoot) && melotts.isModelInstalled(bank.modelDirFor(appRoot, voice));
  }
  return !!env.piper;
}
...
// the callers stay engine-agnostic.
function synthClip(appRoot, voice, text, profile) {
  const baselineWpm = voice.wpmAtScale1 || undefined;
  if (bank.engineOf(voice) === 'melotts') {
    return melotts.synthesize(text, profile, bank.modelDirFor(appRoot, voice), melotts.findEngine(appRoot), {
      baselineWpm,
...
function registerIpcHandlers(appRoot, userDir, hooks = {}) {
  ...
  ipcMain.handle('aegis:bank:download', async (event, voiceId) => {
    ...
    try {
      ...
      // not a crash.
      if (bank.engineOf(voice) === 'melotts' && !melotts.isEngineInstalled(appRoot)) {
        const spec = manifest.engines && manifest.engines.melotts;
        if (!spec) {
          return fail('The HD voice engine is not installed and no download is configured yet.');
        }
        await bank.downloadEngine('melotts', spec, (r, t) => report(r, t, 'engine'));
        lastPct = -1;
      }
