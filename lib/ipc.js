...
function voiceEngineReady(appRoot, voice, env) {
  if (bank.engineOf(voice) === 'melotts') {
    return melotts.isEngineInstalled(appRoot, voice.engineId) && melotts.isModelInstalled(bank.modelDirFor(appRoot, voice));
  }
  return !!env.piper;
}
...
function synthClip(appRoot, voice, text, profile) {
  ...
  if (bank.engineOf(voice) === 'melotts') {
    return melotts.synthesize(text, profile, bank.modelDirFor(appRoot, voice), melotts.findEngine(appRoot, voice.engineId), {
      baselineWpm,
