...
const { ipcMain, dialog, app, shell, nativeImage, clipboard } = require('electron');
...
function registerIpcHandlers(appRoot, userDir, hooks = {}) {
  ...
  // ── Setup Export: one-click share-ready screenshot of the live desktop ──────
  // The capture happens in MAIN (hooks.captureSetup → dashboardWindow.capturePage);
  // here we save / copy / reveal it. The renderer never writes files. Every path is
  // fail-soft, and a cancelled save dialog is a silent no-op (no error toast).
  function defaultSetupPath() {
    let dir;
    try { dir = path.join(app.getPath('pictures'), 'Dashboard Engine'); }
    catch { try { dir = app.getPath('desktop'); } catch { dir = userDir; } }
    try { fs.mkdirSync(dir, { recursive: true }); } catch { /* dialog still opens */ }
    const d = new Date();
    const p2 = (n) => String(n).padStart(2, '0');
    const stamp = `${d.getFullYear()}${p2(d.getMonth() + 1)}${p2(d.getDate())}-${p2(d.getHours())}${p2(d.getMinutes())}${p2(d.getSeconds())}`;
    return path.join(dir, `dashboard-engine-${stamp}.png`);
  }

  // Clamp the renderer-supplied options (assumed hostile) to the known set.
  function setupCaptureOpts(o) {
    const res = o && ['native', '1440p', '1080p'].includes(o.resolution) ? o.resolution : 'native';
    return { resolution: res, watermark: !!(o && o.watermark) };
  }

  ipcMain.handle('aegis:setup:export', async (event, o) => {
    if (typeof hooks.captureSetup !== 'function') return fail('Setup export is unavailable.');
    const cap = await hooks.captureSetup(setupCaptureOpts(o));
    if (!cap || !cap.ok) return fail((cap && cap.error) || 'Could not capture the desktop.');
    const picked = await dialog.showSaveDialog({
      title: 'Export desktop setup',
      defaultPath: defaultSetupPath(),
      filters: [{ name: 'PNG image', extensions: ['png'] }],
    });
    if (picked.canceled || !picked.filePath) return { ok: false, error: null }; // user cancelled → no-op
    try {
      fs.writeFileSync(picked.filePath, cap.image.toPNG());
      return { ok: true, file: picked.filePath };
    } catch (err) {
      return fail(`Could not save the image: ${err.message}`);
    }
  });

  ipcMain.handle('aegis:setup:clipboard', async (event, o) => {
    if (typeof hooks.captureSetup !== 'function') return fail('Setup export is unavailable.');
    const cap = await hooks.captureSetup(setupCaptureOpts(o));
    if (!cap || !cap.ok) return fail((cap && cap.error) || 'Could not capture the desktop.');
    try {
      clipboard.writeImage(cap.image);
      return { ok: true };
    } catch (err) {
      return fail(`Could not copy to the clipboard: ${err.message}`);
    }
  });

  // Reveal a just-saved export in the file manager. Only ever an existing file we wrote.
  ipcMain.handle('aegis:setup:openFolder', (event, filePath) => {
    if (typeof filePath !== 'string' || !filePath) return fail('No file to open.');
    try {
      if (!fs.existsSync(filePath)) return fail('That file no longer exists.');
  ...
  ipcMain.handle('aegis:assistant:config:set', (event, patch) => {
    ...
    if (res.ok) warmupAssistant(userDir);
    return res;
  });
