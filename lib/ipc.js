...
function registerIpcHandlers(appRoot, userDir, hooks = {}) {
  ...
  ipcMain.handle('aegis:bank:download', async (event, voiceId) => {
    ...
    downloadsInFlight.set(voice.id, { pct: 0, phase: 'voice' });
    let lastPct = -1;
    ...
    const report = (received, total, phase) => {
      ...
      downloadsInFlight.set(voice.id, { pct, phase });
      if (pct === lastPct) return;
      lastPct = pct;
      broadcast('aegis:bank:progress', { id: voice.id, received, total, pct, phase });
    };
    try {
  ...
  ipcMain.handle('aegis:bank:inflight', () => ({
    ok: true,
    downloads: [...downloadsInFlight.entries()].map(([id, p]) => ({ id, pct: p.pct, phase: p.phase })),
  }));
  ...
  // A minimal valid profile for a throwaway warm-up synth (audio is discarded).
  ...
  ipcMain.handle('aegis:voice:prewarm', (event, voiceId) => {
    try {
      ...
      if (engine === 'piper') {
        // Piper spawns a fresh process per synth, so there is no persistent warm
        // child — but the slow part of a first cold synth is the OS first-access
        // cost (Windows Defender scanning the piper binary + native libs and the
        // large voice model, plus a cold disk read). A fire-and-forget throwaway
        // synth pays that once in the background so the first real clip is fast.
  ...
  ipcMain.handle('aegis:workshop:publish', async (event, req) => {
    ...
    if (typeof hooks.renderPreview === 'function') {
      // animated = a looping GIF (falls back to a static image in main).
      try { previewPath = await hooks.renderPreview(req.packId, { animated: req.animated === true }); } catch (err) { previewPath = null; }
    }
    try {
  ...
  ipcMain.handle('aegis:workshop:import', (event, itemId) => {
    ...
    const res = workshop.importSubscribed(appRoot, userDir, itemId);
  ...
  // Remaining local publish cooldown (ms) after Steam rate-limited an upload, so
  // the publish dialog can show a concrete "try again in M:SS" countdown.
  ipcMain.handle('aegis:workshop:retryAfter', () => {
    try { return { ok: true, retryAfterMs: workshop.rateLimitRemainingMs(userDir) }; }
    catch (err) { return { ok: true, retryAfterMs: 0 }; }
  });
