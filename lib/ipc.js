...
function splitForSpeech(text, maxLen = SPEAK_CHUNK_CHARS) {
  ...
  for (const raw of sentences) {
    ...
    if (!piece) continue;
    // A single over-long sentence (no terminator) becomes its own clip rather
    // than being force-merged — still bounded by MAX speak length upstream.
...
function registerIpcHandlers(appRoot, userDir, hooks = {}) {
  ...
  // open it in the editor to fine-tune. Only the wallpaper the builder imported
  // (if any) is carried in as an asset — not stray staged images.
  ipcMain.handle('aegis:builder:create', (event, payload) => {
    ...
    const extras = {};
    const wallpaper = rawPack.skin && rawPack.skin.wallpaper;
    if (typeof wallpaper === 'string' && wallpaper) {
      const key = `assets/${path.basename(wallpaper)}`;
  ...
  ipcMain.handle('aegis:assistant:speak', async (event, text) => {
    ...
    try {
      ...
      let sampleRate = piper.sampleRateForModel(modelPath);
      const parts = [];
      for (const clip of clips) {
        const synth = await piper.synthesize(clip, profile, modelPath, piperPath, { baselineWpm });
  ...
  function musicChanged() {
    if (typeof hooks.onMusicChanged === 'function') hooks.onMusicChanged();
  }
  ...
  ipcMain.handle('aegis:music:add', async () => {
    const picked = await dialog.showOpenDialog({
      title: 'Add music',
