...
const llm = require('./llm'); // the bundled local LLM server — the assistant's ONLY model
...
async function checkEnv(appRoot) {
  ...
  const ffmpegPath = dsp.findFfmpeg(appRoot);
...
const PREWARM_PROFILE = { prosody: { rate: 165, expressiveness: 1, steadiness: 0.5, pauseSentence: 200 } };
...
// Preload the bundled local model so the first reply isn't a cold start: spawn
// the llama.cpp server (which loads the ~1 GB model) and give it a tiny throwaway
// request. Fully fail-soft — if the model can't start, the assistant just reports
// "unavailable" when used. appRoot is needed to find the bundled runtime.
// Map the user's Low/Medium/High assistant-speed setting to a llama.cpp thread
// count: higher = faster replies + more CPU. The server also runs below-normal
// priority regardless (lib/llm.js), so the desktop stays responsive at any level.
function assistantLlmThreads(userDir) {
  const n = (require('os').cpus() || []).length || 4;
  const level = settings.getAssistantSpeed(userDir);
  if (level === 'low') return Math.max(2, Math.floor(n / 4));  // light but still usable (never 1 thread)
  if (level === 'high') return Math.max(2, Math.min(8, n - 1));
  return Math.max(2, Math.min(6, n - 2)); // medium (default) — matches llm.js default
}

// The voice half of the Low/Medium/High assistant setting: how long a warmed voice
// engine stays resident when idle before it frees its ~0.4-0.9 GB. Low frees soonest
// (lightest, but the next use pays a cold-start reload); High never frees (always
// warm, no reload — the engine is also preloaded on desktop render for High). Applied
// at startup and whenever the setting changes, so the tier takes effect immediately.
const VOICE_IDLE_MS = { low: 3 * 60 * 1000, medium: 10 * 60 * 1000, high: 0 }; // 0 = never free
function applyVoiceIdlePolicy(userDir) {
  const ms = VOICE_IDLE_MS[settings.getAssistantSpeed(userDir)];
  const window = ms === undefined ? VOICE_IDLE_MS.medium : ms;
  try { melotts.setIdleMs(window); } catch { /* ignore */ }
  try { kokoro.setIdleMs(window); } catch { /* ignore */ }
}

async function warmupAssistant(appRoot, userDir) {
  try {
    // DIAGNOSTIC: wall-clock each warmup sub-step + free RAM, so a "Not Responding"
    // freeze during warmup can be attributed to the LLM load, the voice load, or
    // whole-system starvation (all steps slow → disk/memory pressure, not one call).
    const _t = Date.now();
    const _mem = () => { try { return `freeMB=${Math.round(require('os').freemem() / 1048576)}`; } catch (e) { return ''; } };
    // STAGGER the cold-loads: warm the LLM (~1 GB) first, THEN the voice (~0.9 GB) —
    // never load both at once, which thrashes a low-RAM machine's disk/memory and
    // freezes the desktop on the first "Ask anything" (the Steam "not interactable"
    // report; the log showed free RAM collapsing as both loaded together). The voice
    // warms in the background afterward, while the model generates the first reply, so
    // speech is ready by the time the text finishes.
    const ep = await llm.ensureServer(appRoot, {
      threads: assistantLlmThreads(userDir),
      log: (m) => { try { logger.write(userDir, 'PERF', m); } catch (e) { /* ignore */ } }, // step timing → engine.log
    });
    try { logger.write(userDir, 'PERF', `warmup: llm.ensureServer done +${Date.now() - _t}ms (ep=${!!ep}) ${_mem()}`); } catch (e) { /* ignore */ }
    if (ep) await assistant.warmup(userDir, ep);
    prewarmAssistantVoice(appRoot, userDir); // background voice warm, AFTER the LLM (fire-and-forget)
    try { logger.write(userDir, 'PERF', `warmup: done +${Date.now() - _t}ms ${_mem()}`); } catch (e) { /* ignore */ }
  } catch { /* best effort */ }
}
...
function registerIpcHandlers(appRoot, userDir, hooks = {}) {
  ...
  // Synths share one engine, so only one runs at a time. User-initiated synths
  // (tuning Test, voice preview) REJECT when busy — a double-click shouldn't queue.
  // But the assistant reply + spoken health alerts must never be DROPPED, so they
  // WAIT their turn via this FIFO lock: a launch health alert cold-loading the
  // engine no longer makes the first reply come back 'busy' and go silent.
  const synthWaiters = [];
  function acquireSynthLock() {
    if (!synthesisBusy) { synthesisBusy = true; return Promise.resolve(); }
    return new Promise((resolve) => synthWaiters.push(resolve));
  }
  function releaseSynthLock() {
    const next = synthWaiters.shift();
    if (next) next();          // hand the still-held lock to the next waiter (FIFO)
    else synthesisBusy = false;
  }
  // While the assistant is generating or speaking a reply, its own LLM/TTS load can
  // spike CPU/memory and trip a health alert. That alert would take the single synth
  // lock and the assistant's spoken reply would be skipped (it "talks over" the reply).
  // Suppress health alerts within this window of ANY assistant activity so the reply is
  // never interrupted by an alert caused by the assistant itself.
  let lastAssistantActivityAt = 0;
  const ASSISTANT_QUIET_MS = 4000;
  const markAssistantActive = () => { lastAssistantActivityAt = Date.now(); };
  ...
  // Apply the saved Low/Medium/High tier to the voice engines' idle window now, so a
  // High user's engine stays warm from the first synth even before they touch Settings.
  applyVoiceIdlePolicy(userDir);
  ...
  ipcMain.handle('aegis:test:synthesize', async (event, payload) => {
    ...
    try {
      ...
      if (env.ffmpeg) {
        const wet = await dsp.applyDsp(dryPcm, sampleRate, profile, dsp.findFfmpeg(appRoot));
        pcm = wet.pcm;
  ...
  ipcMain.handle('aegis:packs:load', (event, id, opts) => {
    ...
    // This handler is SYNCHRONOUS and base64s every image asset on the main thread,
    // so a heavy pack can stall the UI. Log a SLOW load (>150 ms) always — it's the
    // prime suspect for a "Not Responding" hang — with full detail under DE_STALLLOG.
    // A fast load stays silent, so this is quiet enough to leave on in production.
    const _total = Date.now() - _t0;
    if (_total > 150 || process.env.DE_STALLLOG) {
      let bytes = 0; for (const v of Object.values(collected.assets)) bytes += (v ? v.length : 0);
      try { logger.write(userDir, 'PERF', `packs:load ${id} total=${_total}ms collectAssets=${Date.now() - _tc}ms assets=${Object.keys(collected.assets).length} base64KB=${Math.round(bytes / 1024)} withProps=${!!(opts && opts.withProps)}`); } catch (e) { /* ignore */ }
    }
    return {
      ok: true,
      pack: loaded.pack,
  ...
  ipcMain.handle('aegis:workshop:browse', async (event, opts) => {
    const o = typeof opts === 'object' && opts !== null ? opts : {};
    const res = await workshop.browse({
      page: Number(o.page) || 1,
      search: typeof o.search === 'string' ? o.search : '',
      sort: typeof o.sort === 'string' ? o.sort : 'trend',
    });
    // Moderation: never surface a blocked item in Browse (import is gated too).
    if (res && res.ok && Array.isArray(res.items)) {
      res.items = res.items.filter((it) => !moderation.isBlocked(appRoot, it.itemId));
    }
    return res;
  });
  ...
  ipcMain.handle('aegis:voice:browse', async (event, opts) => {
    ...
    // Moderation: hide blocked voices in Browse (import is gated too).
    if (res.ok) {
      res.items = res.items
        .filter((it) => !moderation.isBlocked(appRoot, it.itemId))
        .map((it) => ({ ...it, ...baseVoiceInfo(it.baseVoice) }));
    }
    return res;
  });
  ...
  ipcMain.handle('aegis:voice:previewSample', async (event, req) => {
    ...
    try {
      ...
      let pcm = dryPcm;
      if (env.ffmpeg) {
        try { const wet = await dsp.applyDsp(dryPcm, sampleRate, profile, dsp.findFfmpeg(appRoot)); pcm = wet.pcm; }
  ...
  ipcMain.handle('aegis:packs:uninstall', (event, id) => {
    if (typeof id !== 'string') return fail('Invalid pack id.');
    const wasActive = settings.getActivePack(userDir) === id;
    const result = packstore.uninstall(userDir, id);
    // If the desktop was showing this pack, it would keep rendering a pack that no
    // longer exists (the renderer caches it) — switch to the default built-in and
    // repaint so the wallpaper never shows an uninstalled dashboard.
    if (result && result.ok && wasActive) {
      settings.setActivePack(userDir, 'aegis'); // the always-present default pack
      if (typeof hooks.onActivePack === 'function') hooks.onActivePack('aegis');
    }
    return result;
  });
  ...
  ipcMain.handle('aegis:open-editor', (event, id) => {
    ...
    hooks.openEditor(typeof id === 'string' ? id : 'aegis');
    return { ok: true };
  });
  ...
  // ── AI assistant (LOCAL models only — no cloud service, no API key) ───────
  // The base URL is validated to a local/private host in main; there is no key.
  ...
  ipcMain.handle('aegis:assistant:config:set', (event, patch) => {
    ...
    // In the Assistant settings → likely about to test; warm the bundled model.
    if (res.ok) warmupAssistant(appRoot, userDir);
    return res;
  });

  // Preload the local model (Ollama/LM Studio) ahead of the first message so its
  // cold start is hidden. Fired when the chat opens / the desktop loads. No-op
  // until a local model is configured; fully fail-soft.
  ipcMain.handle('aegis:assistant:warmup', async (event, trigger) => {
    // 'render' is a background PRELOAD tied to the response-speed setting: only High
    // preloads on desktop render (fastest, most idle RAM). Fire-and-forget.
    if (trigger === 'render') {
      if (settings.getAssistantSpeed(userDir) === 'high') warmupAssistant(appRoot, userDir);
      return { ok: true };
    }
    // 'open' (and the Manager's untriggered warm): the chatbox GATES on this — warm
    // the model AND the voice engine, resolving only when BOTH are ready, so the
    // first reply has no cold start in the TEXT or the SPEECH. The throwaway synth
    // cold-loads the TTS engine (its PCM is discarded); it shares the FIFO lock, so
    // it queues cleanly behind any in-flight synth. Fast when already warm.
    // Warm the model (awaited so the chat input un-gates when it's ready); the voice
    // warms in the background inside warmupAssistant. The old parallel
    // synthSpeech('Hello.') was a REDUNDANT second voice cold-load piled on the burst —
    // dropped, so the first "Ask anything" no longer loads ~2 GB at once.
    const _tw = Date.now();
    await warmupAssistant(appRoot, userDir);
    try { logger.write(userDir, 'PERF', `warmup('open') total=${Date.now() - _tw}ms`); } catch (e) { /* ignore */ }
    return { ok: true };
  });
  ...
  ipcMain.handle('aegis:assistant:ask', async (event, prompt) => {
    ...
    markAssistantActive(); // hold off health alerts while we generate + speak this reply
    ...
    // Ensure the bundled model server is up (spawns + loads it on first use), then
    // ask it. A null endpoint (model couldn't start) → assistant.ask reports the
    // built-in assistant is unavailable, and we pop the user turn below.
    const ep = await llm.ensureServer(appRoot, { threads: assistantLlmThreads(userDir) });
    const res = await assistant.ask(userDir, assistantThread, (delta) => { markAssistantActive(); send({ id: streamId, delta }); }, ep);
    if (!res.ok) {
  ...
  async function synthSpeech(text) {
    ...
    await acquireSynthLock(); // WAIT for the engine (don't drop a reply behind an alert)
    try {
      const cfg = assistant.getPublicConfig(userDir);
      ...
      const ffmpegPath = env.ffmpeg ? dsp.findFfmpeg(appRoot) : null;
      ...
      for (const clip of clips) {
        let synth;
        try {
          synth = await synthClip(appRoot, voice, clip, profile);
        } catch (err) {
          // A rare, intermittent engine fault (observed with the frozen MeloTTS
          // sidecar on some replies) can reject a clip even though the text is
          // fine — a FRESH engine renders the very same text without error. The
          // engine self-heals on an error frame (lib/melotts tears the bad process
          // down), so retry the clip ONCE on the fresh process before giving up.
          // Log the context — incl. the engine's stderr tail, which carries the
          // Python traceback once the sidecar is built to write one — so a
          // recurrence is diagnosable.
          try {
            const tail = err.engineStderr ? ` | engine-stderr: ${String(err.engineStderr).replace(/\s+/g, ' ').slice(-600)}` : '';
            logger.write(userDir, 'WARN', `[assistant-voice] synth failed for ${voice.id} (${voice.meloLang || voice.engine || ''}); retrying on a fresh engine. err="${err.message}"${tail}`);
          } catch (e) { /* logging is best-effort */ }
          synth = await synthClip(appRoot, voice, clip, profile); // fresh process; throws if it truly can't render
        }
  ...
  ipcMain.handle('aegis:assistant:speak', (event, text) => { markAssistantActive(); return synthSpeech(text); });
  ...
  const healthAlertBy = {}; // metric -> highest severity we've ANNOUNCED and not yet seen clear
  const healthAlertVersions = {};
  let healthVoiceEpoch = 0;

  // Alert wording per voice language — the text is SPOKEN, so it must match the
  // voice that will read it (an English sentence through a Japanese voice is
  // gibberish, exactly like the persona↔voice rule). Names use plain words, not
  // the "C P U" letter-spelling, for the non-Latin engines. es/fr front the metric
  // noun (comma pause) to sidestep gender/article agreement. zh is Simplified —
  // the ZH engine normalizes Traditional→Simplified anyway, and both read alike.
  const HEALTH_ALERT_STRINGS = {
    en: {
      names: { CPU: 'C P U', MEM: 'Memory', DISK: 'Disk', BATTERY: 'Battery', SYSTEM: 'System' },
      usage: (n, s, v) => s === 2 ? `Alert. ${n} usage critical. ${v} percent.` : `Warning. ${n} usage high. ${v} percent.`,
      battery: (s, v) => s === 2 ? `Alert. Battery critical. ${v} percent remaining.` : `Warning. Battery low. ${v} percent.`,
    },
    es: {
      names: { CPU: 'procesador', MEM: 'memoria', DISK: 'disco', BATTERY: 'batería', SYSTEM: 'sistema' },
      usage: (n, s, v) => s === 2 ? `Alerta. ${n}, uso crítico. ${v} por ciento.` : `Advertencia. ${n}, uso elevado. ${v} por ciento.`,
      battery: (s, v) => s === 2 ? `Alerta. Batería crítica. Queda ${v} por ciento.` : `Advertencia. Batería baja. ${v} por ciento.`,
    },
    fr: {
      names: { CPU: 'processeur', MEM: 'mémoire', DISK: 'disque', BATTERY: 'batterie', SYSTEM: 'système' },
      usage: (n, s, v) => s === 2 ? `Alerte. ${n}, utilisation critique. ${v} pour cent.` : `Avertissement. ${n}, utilisation élevée. ${v} pour cent.`,
      battery: (s, v) => s === 2 ? `Alerte. Batterie critique. Il reste ${v} pour cent.` : `Avertissement. Batterie faible. ${v} pour cent.`,
    },
    zh: {
      names: { CPU: '处理器', MEM: '内存', DISK: '磁盘', BATTERY: '电池', SYSTEM: '系统' },
      usage: (n, s, v) => s === 2 ? `警报。${n}使用率过高，百分之${v}。` : `警告。${n}使用率偏高，百分之${v}。`,
      battery: (s, v) => s === 2 ? `警报。电池电量严重不足，剩余百分之${v}。` : `警告。电池电量偏低，百分之${v}。`,
    },
    ja: {
      names: { CPU: 'プロセッサ', MEM: 'メモリ', DISK: 'ディスク', BATTERY: 'バッテリー', SYSTEM: 'システム' },
      usage: (n, s, v) => s === 2 ? `警告。${n}の使用率が危険な水準です。${v}パーセント。` : `注意。${n}の使用率が高くなっています。${v}パーセント。`,
      battery: (s, v) => s === 2 ? `警告。バッテリー残量が危険な水準です。残り${v}パーセント。` : `注意。バッテリー残量が少なくなっています。${v}パーセント。`,
    },
    ko: {
      names: { CPU: '프로세서', MEM: '메모리', DISK: '디스크', BATTERY: '배터리', SYSTEM: '시스템' },
      usage: (n, s, v) => s === 2 ? `경고. ${n} 사용률이 위험 수준입니다. ${v} 퍼센트.` : `주의. ${n} 사용률이 높습니다. ${v} 퍼센트.`,
      battery: (s, v) => s === 2 ? `경고. 배터리 잔량이 위험 수준입니다. ${v} 퍼센트 남았습니다.` : `주의. 배터리 잔량이 부족합니다. ${v} 퍼센트.`,
    },
  };

  // The language of the voice the assistant will actually speak with — so the
  // alert text is composed in that language. The speak-fallback only ever swaps to
  // a SAME-language voice, so the configured voice's language is authoritative.
  function assistantVoiceLang() {
    try {
      const cfg = assistant.getPublicConfig(userDir);
      const profile = resolveVoiceProfile(appRoot, cfg.voiceProfile) || profiles.defaultProfile();
      const voice = bank.voiceById(bank.loadManifest(appRoot), profile.base.voice);
      return voiceLang(voice) || 'en';
    } catch (e) { return 'en'; }
  }

  function composeHealthAlert(metric, severity, value, lang) {
    const v = Math.round(value);
    const key = String(metric || '').toUpperCase();
    const S = HEALTH_ALERT_STRINGS[lang] || HEALTH_ALERT_STRINGS.en;
    const sev = severity === 2 ? 2 : 1;
    if (key === 'BATTERY') return S.battery(sev, v);
    return S.usage(S.names[key] || S.names.SYSTEM, sev, v);
  }
  ...
  ipcMain.handle('aegis:health:alert', async (event, payload) => {
    ...
    if (!metric) return fail('Invalid alert.');
    const raw = Number(payload.severity);
    const severity = raw === 2 ? 2 : raw === 1 ? 1 : 0;
    // A clear (severity 0) carries no speech — it just re-arms the metric so a
    // genuine LATER re-trip is announced again. No setting/level gate needed.
    if (severity === 0) {
      delete healthAlertBy[metric];
      healthAlertVersions[metric] = (healthAlertVersions[metric] || 0) + 1;
      return { ok: false, skipped: 'cleared' };
    }
    if (!settings.getHealthVoiceAlerts(userDir)) return { ok: false, skipped: 'off' };
    // The bundled AI model holds ~1 GB resident and pins the CPU while it runs, so a
    // CPU/memory alert while it's up is the assistant's OWN footprint, not a system
    // problem — don't announce it. (Disk/battery are unrelated and still fire.) It
    // re-arms once the model is freed (e.g. on the performance freeze during a game).
    if ((metric === 'CPU' || metric === 'MEM') && llm.isRunning()) return { ok: false, skipped: 'assistant-load' };
    const value = Number(payload.value);
    if (!Number.isFinite(value)) return fail('Invalid alert.');
    // Speak only when a metric ENTERS a level worse than the one we last announced
    // for it. A metric holding the same level never repeats — a constantly-high
    // disk is announced once, not on a loop; only a real escalation (warn→crit)
    // speaks again. The renderer sends a clear on recovery, which re-arms this.
    const prev = healthAlertBy[metric] || 0;
    if (severity <= prev) return { ok: false, skipped: 'held' };
    // The assistant is generating/speaking — its own load likely caused this spike.
    // Never talk over or preempt the reply: skip WITHOUT recording, so the metric
    // re-evaluates cleanly once the assistant goes quiet (a real, sustained problem
    // re-fires on the next level change).
    if (Date.now() - lastAssistantActivityAt < ASSISTANT_QUIET_MS) return { ok: false, skipped: 'assistant-busy' };
    const version = healthAlertVersions[metric] || 0;
    const epoch = healthVoiceEpoch;
    const out = await synthSpeech(composeHealthAlert(metric, severity, value, assistantVoiceLang()));
    // A recovery or settings toggle during a cold synth invalidates that result.
    if (epoch !== healthVoiceEpoch || version !== (healthAlertVersions[metric] || 0)
      || !settings.getHealthVoiceAlerts(userDir)) return { ok: false, skipped: 'cancelled' };
    if (out.ok) healthAlertBy[metric] = severity;
    return out;
  });
  ...
  // Assistant response speed (Low/Medium/High): trades reply speed for CPU usage
  // by setting the local model's thread count.
  ipcMain.handle('aegis:settings:assistantspeed:get', () => {
    return { ok: true, speed: settings.getAssistantSpeed(userDir), speeds: settings.ASSISTANT_SPEEDS };
  });
  ipcMain.handle('aegis:settings:assistantspeed:set', (event, level) => {
    const speed = settings.setAssistantSpeed(userDir, level);
    // The thread count is fixed at spawn, so stop the running server; the next
    // assistant use lazily respawns it with the new count (no cost until then).
    try { llm.stop(); } catch (e) { /* best effort */ }
    // Re-point the voice engines' idle window at the new tier immediately (High →
    // never free / always warm; Low → free soon). A warm engine re-arms its timer.
    applyVoiceIdlePolicy(userDir);
  ...
  // Hear a sample health alert on demand, in the assistant's current voice — a real
  // alert only fires once per problem, so this is how the user checks the wording +
  // voice + language. Ignores the opt-in gate and the per-metric suppression (it's
  // an explicit test, not a threshold crossing). Returns PCM for the Manager.
  ipcMain.handle('aegis:health:preview', async () => {
    return synthSpeech(composeHealthAlert('DISK', 1, 88, assistantVoiceLang()));
  });

  ipcMain.handle('aegis:settings:healthvoice:set', (event, enabled) => {
    const on = settings.setHealthVoiceAlerts(userDir, Boolean(enabled));
    healthVoiceEpoch += 1;
    broadcast('aegis:healthvoice:changed', on);
    if (on) {
      // Enabling mid-session should announce anything ALREADY in alert. Forget the
      // per-metric memory and tell the desktop to re-report current levels, so a
      // metric that's been sitting in the orange the whole time is heard once now.
      for (const k of Object.keys(healthAlertBy)) delete healthAlertBy[k];
      if (typeof hooks.onHealthVoiceEnabled === 'function') hooks.onHealthVoiceEnabled();
    }
  ...
  // Main's pinDisplay() records the monitor's full geometry signature (not just
  // the id) so the choice survives an id shuffle across a reboot.
  ipcMain.handle('aegis:settings:display:set', (event, id) => {
    if (id !== null && !Number.isInteger(id)) return fail('Invalid display.');
    if (typeof hooks.pinDisplay === 'function') hooks.pinDisplay(id);
