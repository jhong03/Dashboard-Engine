 ...
 */
// Strip personal / machine-specific values a designer may have filled in that
// must NOT travel in a SHARED pack (publish or export). Design content — text,
// labels, countdown dates, persona, colours — is intentionally kept; that's what
// a shared dashboard IS. Personal DATA is reset to safe defaults so a
// subscriber's copy uses THEIR data, never the author's:
//   • weather lat/lon/place → 0/0/null, so the widget follows each subscriber's
//     own Settings weather location instead of revealing where the author lives.
// (Reminders/calendar events, launcher pins, notifications, now-playing, the
//  assistant chat and system stats are never stored in a pack — they render live
//  from the viewer's own machine — so nothing else here needs stripping.)
// Returns a deep clone; the author's local pack is never mutated.
function sanitizeForShare(pack) {
  let clean;
  try { clean = JSON.parse(JSON.stringify(pack)); } catch (e) { return pack; }
  if (Array.isArray(clean.components)) {
    for (const c of clean.components) {
      if (!c || typeof c !== 'object' || !c.options) continue;
      if (c.type === 'weather') { c.options.lat = 0; c.options.lon = 0; c.options.place = null; }
    }
  }
  return clean;
}
