 ...
 */
// Allocate a pack id used by NEITHER a built-in nor an installed pack, appending
// -2, -3, … on collision. Install-from-file uses this so re-importing a pack you
// already have makes a SEPARATE copy instead of overwriting it.
function uniquePackId(appRoot, userDir, baseId) {
  const taken = (candidate) =>
    fs.existsSync(path.join(packs.packDir(appRoot, candidate), 'pack.json')) ||
    fs.existsSync(path.join(userPacksDir(userDir), candidate, 'pack.json'));
  if (!taken(baseId)) return baseId;
  for (let n = 2; n < 1000; n++) {
    const candidate = `${baseId}-${n}`.slice(0, 48);
    if (!taken(candidate)) return candidate;
  }
  return `${baseId}-${Date.now()}`.slice(0, 48);
}
...
function installFromBuffer(appRoot, userDir, buffer, origin = {}) {
  ...
  if (origin.freshCopy) {
    // Install-from-file is always an independent NEW copy: never overwrite a built-in
    // OR an existing user pack (which would clobber the user's own master), and never
    // inherit a publishable "master" identity. Allocate a fresh id; on a collision,
    // disambiguate the display name too so the copy isn't a look-alike.
    const freshId = uniquePackId(appRoot, userDir, id);
    if (freshId !== id) {
      const suffix = freshId.slice(id.length).replace(/^-/, '');
      if (typeof raw.name === 'string' && raw.name.trim()) {
        raw.name = `${raw.name} (${suffix})`.slice(0, 60);
        entries.set('pack.json', Buffer.from(JSON.stringify(raw, null, 2), 'utf8'));
      }
    }
    id = freshId;
    origin = { ...origin, origin: 'imported' }; // a file import is never the master copy
  } else if (fs.existsSync(path.join(packs.packDir(appRoot, id), 'pack.json'))) {
