...
function createPack(appRoot, userDir, rawPack, extraAssets = {}) {
  ...
  try {
    ...
    fs.writeFileSync(path.join(tmpDir, META_FILE), JSON.stringify({
      installedAt: new Date().toISOString(), source: 'editor', version: null, forkedFrom: null,
    }, null, 2));
    fs.rmSync(finalDir, { recursive: true, force: true });
    fs.renameSync(tmpDir, finalDir);
  } catch (err) {
...
module.exports = { userPacksDir, installFromBuffer, exportPack, uninstall, readMeta, saveEdited, createPack, sha256 };
