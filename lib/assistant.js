...
const THREAD_FILE = 'assistant-thread.json'; // persisted conversation (personal data — never in a pack)
const MAX_THREAD_TURNS = 40;                  // scrollback we keep on disk
...
function parseOpenAIShape(status, ok, data) {
  ...
  // Reasoning models embed a chain-of-thought in <think>…</think> (or leave a
  // dangling …</think> when they run long) — keep only the actual answer.
...
module.exports = {
  defaults, getPublicConfig, saveConfig, loadRaw, decryptKey, ask, isConfigured, PROVIDERS,
