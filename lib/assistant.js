...
// The model is BUNDLED and installed with the app, so the assistant is available
// out of the box — no user setup, no endpoint to configure. A machine that can't
// actually run the model surfaces "unavailable" at ask time (see ask() and
// lib/llm ensureServer), not here.
...
async function callOpenAICompatible(config, messages, signal, endpoint) {
  if (!endpoint || !endpoint.baseUrl) return { ok: false, error: 'The built-in assistant is unavailable right now.' };
  const url = `${endpoint.baseUrl.replace(/\/+$/, '')}/chat/completions`;
  // A ceiling on the reply. The bundled Qwen2.5 is NOT a reasoning model, so it
  // stops on its own after a short answer — no large "thinking" floor needed.
  const budget = Number(config.maxTokens) || 1024;
...
async function streamOpenAICompatible(config, messages, signal, onDelta, endpoint) {
  if (!endpoint || !endpoint.baseUrl) return { ok: false, error: 'The built-in assistant is unavailable right now.' };
  const url = `${endpoint.baseUrl.replace(/\/+$/, '')}/chat/completions`;
  const budget = Number(config.maxTokens) || 1024;
  const res = await fetch(url, {
 ...
 */
async function ask(userDir, messages, onDelta, endpoint) {
  const { config } = loadRaw(userDir);
  // The model is bundled and started by lib/llm (the caller passes its endpoint).
  // A null endpoint means the local server couldn't start on this machine (too
  // little memory / an unsupported CPU) — say so; the rest of the app is fine.
  if (!endpoint || !endpoint.baseUrl) {
    return {
      ok: false,
      error: 'The built-in assistant could not start on this machine — it needs enough memory and a modern CPU. Everything else works normally.',
    };
  }
  ...
  try {
    ...
    return await callOpenAICompatible(config, windowed, controller.signal, endpoint);
  } catch (err) {
...
async function warmup(userDir, endpoint) {
  try {
    if (!endpoint || !endpoint.baseUrl) return; // lib/llm couldn't start the server
