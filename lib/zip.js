...
const MAX_ENTRY_BYTES = 5 * 1024 * 1024;         // one image asset
const MAX_VIDEO_ENTRY_BYTES = 30 * 1024 * 1024;  // one video wallpaper (mp4/webm)
const MAX_TOTAL_BYTES = 55 * 1024 * 1024;        // whole pack, uncompressed
                                                 // (one 30 MB video + ~25 MB images)

// What may live inside a pack archive. Deliberately identical in spirit to
// lib/packs.js ASSET_PATTERN/VIDEO_PATTERN — the archive can't smuggle what the
// sanitizer would reject anyway, but rejecting at the container keeps junk off
// disk. Video entries get their own (larger) per-entry cap below.
const ENTRY_PATTERN = /^(pack\.json|assets\/[a-z0-9._-]+\.(png|jpg|jpeg|webp|mp4|webm))$/i;
const VIDEO_ENTRY_PATTERN = /^assets\/[a-z0-9._-]+\.(mp4|webm)$/i;
...
function readZip(buf) {
  ...
  for (let i = 0; i < Math.min(count, MAX_ENTRIES); i++) {
    ...
    // Video wallpapers get the larger cap; images (and pack.json) keep 5 MB.
    const entryCap = VIDEO_ENTRY_PATTERN.test(name) ? MAX_VIDEO_ENTRY_BYTES : MAX_ENTRY_BYTES;
    if (uncompSize > entryCap) {
      warnings.push(`Skipped "${name}" — ${(uncompSize / 1048576).toFixed(1)} MB exceeds the ${entryCap / 1048576} MB per-file cap.`);
      continue;
    }
    ...
    try {
      if (method === 0) {
        ...
        // Cap inflation at this entry's own limit so a video's larger cap never
        // widens the zip-bomb ceiling for images.
        data = zlib.inflateRawSync(raw, { maxOutputLength: entryCap });
      } else {
