...
// Estimate a "word" count that's comparable across languages, so the measured
// rate lands in the same units the Speed slider targets (piper.js baseline =
// 165 wpm at scale 1). Space-delimited languages (English/Spanish/French, and
// Korean — hangul uses spaces) count real words. Chinese/Japanese have NO word
// spaces, so a whitespace split counts the whole sentence as ~1 word and the rate
// collapses to ~10 wpm — the bug users saw. There we estimate word-equivalents
// from the character count: Japanese counts morae (kana ~1, kanji ~1-2) so it
// needs a larger divisor than Chinese (each hanzi ~1 syllable). The divisors are
// chosen so NATURAL CJK speech measures near the 165 baseline — i.e. the measured
// rate tracks the slider instead of reading as gibberish.
const CJK_NO_SPACE_RE = /[㐀-鿿぀-ヿｦ-ﾟ]/g; // CJK ideographs + kana (NOT hangul)
const KANA_RE = /[぀-ヿｦ-ﾟ]/;
function estimateWordCount(text) {
  const t = String(text).trim();
  if (!t) return 0;
  const cjk = t.match(CJK_NO_SPACE_RE);
  if (!cjk || cjk.length === 0) return t.split(/\s+/).filter(Boolean).length;
  const perWord = KANA_RE.test(t) ? 2.5 : 1.8; // Japanese morae vs Chinese syllables
  // Count any interleaved Latin words (e.g. "PC") too, then add the CJK estimate.
  const latinWords = t.replace(CJK_NO_SPACE_RE, ' ').split(/\s+/).filter(Boolean).length;
  return latinWords + cjk.length / perWord;
}
 ...
 * Language-aware word count so Chinese/Japanese don't collapse to ~10 wpm.
 */
function wordsPerMinute(text, speechSeconds) {
  if (speechSeconds <= 0) return 0;
  return (estimateWordCount(text) / speechSeconds) * 60;
}
