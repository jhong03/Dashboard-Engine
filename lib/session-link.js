...
// app binary.
function connect(execPath, intent, { onEnd, onFail, onRpc }) {
  let settled = false;
  ...
  const spawnEngine = () => {
    ...
    // one thing that has to be validated on the real Steam client; if it doesn't, we
    // escalate this spawn to a job-breakaway launcher.
  ...
  const tryConnect = () => {
    ...
    socket.on('connect', () => {
      ...
      const feed = lineReader((msg) => {
        if (msg.cmd === 'session-end') { finish(onEnd); return; } // Manager closed
        if (msg.cmd === 'rpc') {
