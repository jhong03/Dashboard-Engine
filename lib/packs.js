...
// Live values a meter/sparkline may bind to (sampled in the main process).
const BINDS = ['cpu', 'mem'];
...
const COMPONENT_SANITIZERS = {
  ...
  stats(options) {
    return { cpu: bool(options.cpu, true), mem: bool(options.mem, true) };
  },
  meter(options) {
    return {
...
function assetDataUri(appRoot, pack, relPath, budget) {
 ...
 * data: URIs, deduped, within a total byte budget.
 * @returns {{ assets: Object<string,string>, warnings: string[] }}
 */
function collectAssets(appRoot, pack) {
  ...
  for (const relPath of wanted) {
    const result = assetDataUri(appRoot, pack, relPath, budget);
