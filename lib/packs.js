...
function int(value, min, max, fallback) {
  // num() returns the fallback (often null = "inherit") for invalid input; round
  // only a real number, so int(null,…,null) stays null instead of Math.round(null)=0
  // — which was silently turning inherit-padding into an explicit 0 on every save.
...
const COMPONENT_SANITIZERS = {
  ...
  // Breathing rig (Phase F): layered PNGs (back-to-front, ≤8) that come alive
  // with per-layer oscillators — breath (scale), sway (rotate), bob (translate)
  // — plus pointer-driven gaze/tilt. No bones, transform-only. Every knob is
  // clamped to the range the editor exposes and the renderer expects.
  rig(options, warnings) {
    const raw = Array.isArray(options.layers) ? options.layers : [];
    if (raw.length > 8) warnings.push(`Rig has ${raw.length} layers; only the first 8 are kept.`);
    const layers = [];
    for (const rl of raw.slice(0, 8)) {
      if (typeof rl !== 'object' || rl === null) continue;
      const src = assetPath(rl.src);
      if (!src) { warnings.push('Dropped a rig layer — src must be assets/<name>.(png|jpg|webp) inside the pack.'); continue; }
      const anchor = (typeof rl.anchor === 'object' && rl.anchor !== null) ? rl.anchor : {};
      const breath = (typeof rl.breath === 'object' && rl.breath !== null) ? rl.breath : {};
      const sway = (typeof rl.sway === 'object' && rl.sway !== null) ? rl.sway : {};
      const bob = (typeof rl.bob === 'object' && rl.bob !== null) ? rl.bob : {};
      const gaze = (typeof rl.gaze === 'object' && rl.gaze !== null) ? rl.gaze : {};
      layers.push({
        src,
        anchor: { x: num(anchor.x, 0, 100, 50), y: num(anchor.y, 0, 100, 90) },
        breath: { scale: num(breath.scale, 0, 0.05, 0), speed: num(breath.speed, 0.05, 1, 0.2), phase: num(breath.phase, 0, 1, 0) },
        sway: { rotate: num(sway.rotate, 0, 6, 0), speed: num(sway.speed, 0.05, 1, 0.2), phase: num(sway.phase, 0, 1, 0) },
        bob: { y: num(bob.y, 0, 3, 0), speed: num(bob.speed, 0.05, 1, 0.2), phase: num(bob.phase, 0, 1, 0) },
        gaze: { x: num(gaze.x, 0, 4, 0), y: num(gaze.y, 0, 4, 0) },
        tiltWithPointer: num(rl.tiltWithPointer, 0, 4, 0),
      });
    }
    return { layers };
  },
...
// skin.schedule = four fixed slots (dawn/day/dusk/night), each with a start hour
// and a PARTIAL palette override (only the keys it names change; the rest inherit
// the base palette). Only emitted when the pack authors a schedule, so packs
// without one stay byte-identical. The renderer ignores it unless `enabled`.
const SCHEDULE_SLOTS = [['dawn', 5], ['day', 8], ['dusk', 17], ['night', 20]];

function sanitizeSlotPalette(raw) {
  const src = (typeof raw === 'object' && raw !== null) ? raw : {};
  const out = {};
  for (const key of PALETTE_KEYS) {
    const c = color(src[key], null);
    if (c) out[key] = c;
  }
  return out;
}
...
function sanitizeSchedule(raw) {
  if (typeof raw !== 'object' || raw === null) return null;
  const slotsRaw = (typeof raw.slots === 'object' && raw.slots !== null) ? raw.slots : {};
  const slots = {};
  for (const [name, defHour] of SCHEDULE_SLOTS) {
    const sr = (typeof slotsRaw[name] === 'object' && slotsRaw[name] !== null) ? slotsRaw[name] : {};
    slots[name] = { startHour: int(sr.startHour, 0, 23, defHour), palette: sanitizeSlotPalette(sr.palette) };
  }
  return { enabled: bool(raw.enabled, false), slots };
}

// ── Keyframe timeline (Phase G) ──────────────────────────────────────────────
// pack.timeline animates a whitelist of numeric targets over a looping duration.
// Each target range is [min, max, default]; a key value is clamped to it. Tracks
// referencing a missing component index are dropped (not fatal). Only emitted
// when a pack authors a valid timeline.
const TIMELINE_COMPONENT_PROPS = {
  opacity: [0, 1, 1],
  x: [-50, 50, 0],
  y: [-50, 50, 0],
  scale: [0, 3, 1],
...
const TIMELINE_EASES = ['linear', 'in', 'out', 'inout'];
const TIMELINE_LOOPS = ['loop', 'mirror', 'once'];
const MAX_TIMELINE_TRACKS = 8;
const MAX_TIMELINE_KEYS = 6;

function sanitizeTimelineTarget(raw, componentCount, warnings) {
  if (typeof raw !== 'object' || raw === null) return null;
  const kind = pick(raw.kind, ['component', 'ambience'], null);
  if (kind === 'ambience') return { kind: 'ambience', prop: 'opacity' };
  if (kind === 'component') {
    // Validate (don't clamp) — an out-of-range index must be DROPPED, not snapped
    // to the nearest real component, or a stale track would silently retarget.
    const index = Number(raw.index);
    if (!Number.isInteger(index) || index < 0 || index >= componentCount) {
      warnings.push(`Dropped a timeline track — component #${raw.index} is out of range.`);
      return null;
    }
    const prop = pick(raw.prop, Object.keys(TIMELINE_COMPONENT_PROPS), null);
    if (!prop) return null;
    return { kind: 'component', index, prop };
  }
  return null;
}

function sanitizeTimeline(raw, componentCount, warnings) {
  if (typeof raw !== 'object' || raw === null) return null;
  const duration = num(raw.duration, 1, 300, 10);
  const loop = pick(raw.loop, TIMELINE_LOOPS, 'loop');
  const rawTracks = Array.isArray(raw.tracks) ? raw.tracks : [];
  if (rawTracks.length > MAX_TIMELINE_TRACKS) {
    warnings.push(`Timeline has ${rawTracks.length} tracks; only the first ${MAX_TIMELINE_TRACKS} are kept.`);
  }
  const tracks = [];
  for (const rt of rawTracks.slice(0, MAX_TIMELINE_TRACKS)) {
    if (typeof rt !== 'object' || rt === null) continue;
    const target = sanitizeTimelineTarget(rt.target, componentCount, warnings);
    if (!target) continue;
    const range = target.kind === 'ambience' ? [0, 1, 1] : TIMELINE_COMPONENT_PROPS[target.prop];
    const rawKeys = Array.isArray(rt.keys) ? rt.keys : [];
    const keys = [];
    for (const rk of rawKeys.slice(0, MAX_TIMELINE_KEYS)) {
      if (typeof rk !== 'object' || rk === null) continue;
      const kt = num(rk.t, 0, duration, null);
      if (kt === null) continue;
      keys.push({ t: kt, v: num(rk.v, range[0], range[1], range[2]), ease: pick(rk.ease, TIMELINE_EASES, 'linear') });
    }
    if (!keys.length) { warnings.push('Dropped a timeline track with no valid keyframes.'); continue; }
    keys.sort((a, b) => a.t - b.t);
    tracks.push({ target, keys });
  }
...
function sanitizePack(raw, packId) {
  ...
  // Time-of-day schedule (Phase G): per-slot palette overrides, crossfaded at
  // runtime. Emitted only when the pack authors one → existing packs unchanged.
  const schedule = sanitizeSchedule(skin.schedule);
  if (schedule) clean.skin.schedule = schedule;

  // Keyframe timeline (Phase G): animates a whitelist of numeric targets on the
  // ONE shared loop. Needs the final component count to validate track targets.
  const timeline = sanitizeTimeline(raw.timeline, clean.components.length, warnings);
...
function collectAssets(dir, pack) {
  ...
  for (const component of pack.components) {
    ...
    // Breathing rig (Phase F): every layer PNG is a pack image.
    if (component.type === 'rig' && Array.isArray(component.options.layers)) {
      for (const layer of component.options.layers) if (layer && layer.src) wanted.add(layer.src);
    }
  }
