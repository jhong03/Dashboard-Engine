...
function collectAssets(dir, pack) {
  ...
  // A video wallpaper never enters the base64 path — it streams over depack://
  // (see collectVideoRefs / lib/videostore). Only image wallpapers go here.
  if (pack.skin.wallpaper && !isVideoAsset(pack.skin.wallpaper)) wanted.add(pack.skin.wallpaper);
...
function collectVideoRefs(pack) {
  const refs = new Set();
  if (pack.skin.wallpaper && isVideoAsset(pack.skin.wallpaper)) refs.add(pack.skin.wallpaper);
  return [...refs];
}
