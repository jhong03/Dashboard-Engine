...
- **AI assistant / LLM bridge** (`lib/assistant.js` + `src/assistant.*` + `preload-assistant.js`): BYO API key — Anthropic (Claude Messages API) or any OpenAI-compatible endpoint (base URL + key). Key stored OS-encrypted via Electron `safeStorage`, never exposed to a renderer (only `hasKey`) or written into a pack/export. Configured in the Manager → Assistant tab (provider/key/model/persona/voice/speak + Test). The chat is a focusable window (`createAssistantWindow`) summoned by clicking the desktop `assistant` console — the desktop surface is `focusable:false` so it can't take keyboard input itself. Replies are spoken through the tuning voice pipeline (`aegis:assistant:speak` reuses piper/dsp with the chosen saved profile). Conversation state lives in main (`aegis:assistant:ask`, `:reset`). No hosted free tier — that would need a proxy server (future).
...
- LLM bridge SHIPPED 2026-07-13 (BYO-key chat, Anthropic + OpenAI-compatible, voice-spoken replies, desktop console). Still open: a hosted free-tier default (needs a proxy server), response streaming, and the rest of the service widgets — Spotify/YouTube media, audio visualizer, Bluetooth nearby, net-rate telemetry (each needs real infra: OAuth token storage, audio loopback, etc.)
