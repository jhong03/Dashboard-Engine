...
- A **seed gallery** of built-in packs demonstrates range for new users (each must clear the JARVIS quality floor, distinct aesthetic + persona + component coverage): `hearth` (warm/cozy, serif, embers), `slate` (minimal, monochrome). Planned: cute/fluffy, dark/emo, expressive, anime-*aesthetic* (NO copyrighted characters in public defaults — original/abstract art only). Font limit: only 4 built-in stacks (no custom fonts in packs) — palette/shape/ambience carry mood; adding system-font stacks is the lever for cute/expressive.
